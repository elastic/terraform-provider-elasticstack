...
test('code-factory-issue workflow template drives prompt from normalized outputs', () => {
  ...
  assert.match(workflowTmpl, /needs\.pre_activation\.outputs\.sanitized_issue_body/);
  assert.match(workflowTmpl, /needs\.pre_activation\.outputs\.sanitized_issue_comments/);
});
