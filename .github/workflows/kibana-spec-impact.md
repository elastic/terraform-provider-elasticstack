...
5. Write `kibana-spec-impact-issued.json` as a JSON array of Terraform `entity_name` strings for which you **actually created** an issue this run (≤ issue cap). If you created zero issues, write `[]`. **Do not** list entities skipped due to the cap unless you opened an issue for them.
6. Persist repo memory by running (this **always** advances the analyzed baseline to `target_sha`; dedupe fingerprints are recorded **only** for entities listed in `--issued`, so capped entities stay eligible next run):
   ```
...
- Only list entities in `kibana-spec-impact-issued.json` when you created their issue; otherwise dedupe state must not record them (avoids suppressing never-filed entities).
