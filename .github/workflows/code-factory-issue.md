...
You implement exactly one GitHub issue labeled `code-factory`. The triggering issue is the sole source of truth for scope, acceptance criteria, and requested behavior. Do not broaden the scope beyond what the issue describes unless the repository already requires it to make the implementation viable.
...
3. Open exactly one pull request for that branch using the `create-pull-request` safe output.
4. Preserve canonical issue linkage metadata for deterministic reruns by including `Closes #${{ github.event.issue.number }}` in the PR body — this is the stable identifier that prevents duplicate PR creation on future workflow runs.
5. Keep the pull request labeled `code-factory`.
