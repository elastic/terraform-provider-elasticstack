...
The Elastic Stack is available in the agent environment. It is exposed on AWF-allowed host ports `8080` (Elasticsearch) and `80` (Kibana), reachable via `host.docker.internal` from the sandbox. If `terraform` is not on PATH inside the agent container, it is also available at `./.bin/terraform` relative to the workspace. Run targeted acceptance tests with:
