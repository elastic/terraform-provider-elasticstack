...
9. **Commit and push**:
   ...
   - Check whether a PR already exists for the `generated-changelog` branch (use the GitHub MCP tools to search open PRs, or check with `git ls-remote --heads origin generated-changelog`).
   - **If no existing PR**: use `create-pull-request` to create a new PR from the `generated-changelog` branch targeting `main`:
   ...
   - **If a PR already exists** for the `generated-changelog` branch: use `create-pull-request` to update the branch with the new changes (the framework will force-push to the existing branch, automatically updating the open PR). Additionally use `update-pull-request` to refresh the PR description with the new date and compare range.
