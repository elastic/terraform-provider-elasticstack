...
const workflowPath = path.resolve(
	__dirname,
	"../../../workflows/openspec-verify-label.md",
);
...
test("verify-label workflow prompt does not instruct remove-labels cleanup", () => {
	...
	assert.match(
		source,
		/do \*\*not\*\* emit \*\*`remove-labels`\*\* safe outputs/,
	);
});
...
test("verify-label workflow exposes review disposition and disposition reason to the agent", () => {
	...
	assert.match(
		source,
		/\*\*Review disposition\*\*.*approval-eligible.*comment-only/s,
	);
...
test("verify-label agent prompt interpolates needs.pre_activation review outputs where the agent reads them", () => {
	...
	assert.ok(
		source.includes(rd),
		"expected review_disposition interpolation in generated workflow",
	);
	assert.ok(
		source.includes(dr),
		"expected disposition_reason interpolation in generated workflow",
	);
	const pre = source
		.split("## Pre-activation context")[1]
		.split("## Verification (active change)")[0];
	assert.ok(
		pre.includes(rd),
		"expected review_disposition in Pre-activation context",
	);
	assert.ok(
		pre.includes(dr),
		"expected disposition_reason in Pre-activation context",
	);
	const step5 = source
		.split("## Review body, inline comments, and decision")[1]
		.split("## Archive and push")[0];
	assert.ok(
		step5.includes(rd),
		"expected review_disposition in review-submission instructions",
	);
});
...
test("verify-label agent prompt interpolates archive/push outputs where the agent reads them", () => {
	...
	const archiveAllowed =
		"${{ needs.pre_activation.outputs.archive_push_allowed }}";
	const archiveAllowedReason =
		"${{ needs.pre_activation.outputs.archive_push_allowed_reason }}";
	assert.ok(
		source.includes(archiveAllowed),
		"expected archive_push_allowed interpolation in generated workflow",
	);
	assert.ok(
		source.includes(archiveAllowedReason),
		"expected archive_push_allowed_reason interpolation in generated workflow",
	);
	const pre = source
		.split("## Pre-activation context")[1]
		.split("## Verification (active change)")[0];
	assert.ok(
		pre.includes(archiveAllowed),
		"expected archive_push_allowed in Pre-activation context",
	);
	assert.ok(
		pre.includes(archiveAllowedReason),
		"expected archive_push_allowed_reason in Pre-activation context",
	);
});
...
test("verify-label workflow states archive_push_allowed false does not force COMMENT review", () => {
	...
	assert.match(
		source,
		/archive_push_allowed.*false.*does \*\*not\*\* force \*\*`COMMENT`\*\*/s,
	);
});
...
test("verify-label compiled lock pre_activation job has issues and pull-requests write permissions", () => {
	...
	const preActivationMatch = lock.match(
		/^ {2}pre_activation:\n([\s\S]*?)(?=\n {2}\w[\w-]*:\n)/m,
	);
	assert.ok(
		preActivationMatch,
		"expected to find pre_activation job block in lock file",
	);
	const preActivationBlock = preActivationMatch[0];
	assert.ok(
		preActivationBlock.includes("issues: write"),
		"expected issues: write in pre_activation job permissions",
	);
	assert.ok(
		preActivationBlock.includes("pull-requests: write"),
		"expected pull-requests: write in pre_activation job permissions",
	);
});
...
test("verify-label compiled lock has no verification_mode conditions on any steps", () => {
	...
	const workspaceGuard =
		/if:.*needs\.pre_activation\.outputs\.verification_mode/;
