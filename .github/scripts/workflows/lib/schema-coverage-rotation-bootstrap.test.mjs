...
const workflowPath = path.resolve(
	__dirname,
	"../../../workflows/schema-coverage-rotation.md",
);
const lockPath = path.resolve(
	__dirname,
	"../../../workflows/schema-coverage-rotation.lock.yml",
);
...
function extractRepositoryToolchainSection(source) {
	...
	assert.ok(
		startIdx !== -1,
		`missing heading ${JSON.stringify(startMarker)} in ${workflowPath}`,
	);
...
test("schema-coverage rotation workflow uses Claude through OpenRouter with secret-backed API key and tool timeout", () => {
	const source = workflowSource();
...
test("schema-coverage rotation source workflow configures engine env with OpenRouter base URL and model", () => {
	const source = workflowSource();
	assert.match(source, /engine:\s*\n\s*id:\s*claude/m);
	assert.match(source, /model: "?anthropic\/claude-sonnet-5"?/);
	assert.match(
