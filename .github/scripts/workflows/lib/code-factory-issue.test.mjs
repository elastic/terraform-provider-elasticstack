...
const { ISSUE_BRANCH_PREFIX, FACTORY_LABEL, DUPLICATE_LINKAGE_MODE } = require('./intake/code-factory-constants.js');
...
test('code-factory-issue workflow source exists and wires script modules', () => {
...
test('code-factory-issue lock file is compiled and exists', () => {
  const lock = normalizeCompiledLock(readFileSync(lockPath, 'utf8'));
...
test('code-factory intake constants stay aligned with workflow source branch prefix', () => {
  ...
  assert.ok(
    ...
    'code-factory-issue.md must express branches with ISSUE_BRANCH_PREFIX + ${{ needs.pre_activation.outputs.issue_number }}',
  );
});

test('code-factory-issue workflow source enables status comments and remove-label pre-activation', () => {
