...
func createSpace(ctx context.Context, client *clients.KibanaScopedClient, _ string, plan resourceModel) (resourceModel, diag.Diagnostics) {
	...
	_, sdkDiags := kibanaoapi.CreateSpace(ctx, oapiClient, body)
...
func solutionForPostBody(v types.String) *kbapi.PostSpacesSpaceJSONBodySolution {
	...
	s := kbapi.PostSpacesSpaceJSONBodySolution(v.ValueString())
	return &s
}

func solutionForPutBody(v types.String) *kbapi.PutSpacesSpaceIdJSONBodySolution {
	if v.IsNull() || v.IsUnknown() || v.ValueString() == "" {
		return nil
	}
	s := kbapi.PutSpacesSpaceIdJSONBodySolution(v.ValueString())
	return &s
}
