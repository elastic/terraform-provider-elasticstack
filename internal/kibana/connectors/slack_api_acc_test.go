...
// `name`. Kibana echoes the channels back and the provider remarshals the read
// response through kbapi.SlackApiConfig, whose channel `Id` field lacks
// omitempty, so each channel gains an empty `"id":""`. Because .slack_api had no
// `defaults` handler, the config attribute's semantic-equality check did not
// normalize the planned value the same way, so the planned value (no id) and the
// value read back after apply (id="") diverged and Terraform failed with
// "Provider produced inconsistent result after apply". The fix wires the typed
// remarshal in as the .slack_api defaults function so both sides normalize
// identically. See internal/clients/kibanaoapi/connector_defaults.go.
