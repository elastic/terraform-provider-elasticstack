...
func Test_legacyMetricConfigModel_toAPI_ESQL_withQuery(t *testing.T) {
	model := &legacyMetricConfigModel{
		...
		MetricJSON:  customtypes.NewJSONWithDefaultsValue[map[string]any](`{"operation":"value","column":"y","format":{"type":"number"},"color":{"type":"static","color":"#fff"}}`, populateLegacyMetricMetricDefaults),
	}
	_, diags := model.toAPI()
	require.True(t, diags.HasError())
