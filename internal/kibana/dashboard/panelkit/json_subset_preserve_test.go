...
// Regression: practitioner by-value chart JSON vs. Kibana GET expansion (metric panel).
func TestPriorJSONEmbeddedInExpandedCurrent_terraformAndKibanaSample(t *testing.T) {
	t.Parallel()
	...
	ok, err := PriorJSONEmbeddedInExpandedCurrent(prior, api)
	require.NoError(t, err)
	require.True(t, ok, "expected prior to be a value-subset of api for preservation")
}

func TestPriorJSONEmbeddedInExpandedCurrent_rejectsWithoutChartType(t *testing.T) {
	t.Parallel()
	ok, err := PriorJSONEmbeddedInExpandedCurrent(`{"title":"x"}`, `{"title":"x","type":"metric"}`)
	require.NoError(t, err)
	require.False(t, ok)
}
