...
func TestConverter_roundTrip_NoESQL(t *testing.T) {
	...
	var chart kbapi.KibanaHTTPAPIsWaffleChart
	require.NoError(t, chart.FromKibanaHTTPAPIsWaffleNoESQL(waffle))
	var attrs lenscommon.LensByValueConfig
	require.NoError(t, attrs.Chart.FromKibanaHTTPAPIsWaffleChart(chart))
	...
	chart, err := attrs2.Chart.AsKibanaHTTPAPIsWaffleChart()
	require.NoError(t, err)
	noESQL2, err := chart.AsKibanaHTTPAPIsWaffleNoESQL()
	require.NoError(t, err)
