...
const (
	xyLayerTypeDescription = "The type of layer. Valid values: 'area', 'line', 'bar', 'horizontal_bar', 'reference_lines' for NoESQL layers; " +
		"'area_chart', 'line_chart', 'bar_chart', 'horizontal_bar_chart', 'reference_lines' for ESQL layers."
...
func xyAxisSchemaAttrs() map[string]schema.Attribute {
...
func xyDecorationsSchemaAttrs() map[string]schema.Attribute {
	return map[string]schema.Attribute{
...
func xyLegendSchemaAttrs() map[string]schema.Attribute {
	return map[string]schema.Attribute{
...
func xyChartConfigSchemaAttrs(includePresentation bool) map[string]schema.Attribute {
	attrs := map[string]schema.Attribute{
		...
		"fitting": schema.SingleNestedAttribute{
			...
			Required:            true,
			Attributes:          xyFittingSchemaAttrs(),
		},
		"layers": schema.ListNestedAttribute{
			...
			Required:            true,
			NestedObject:        xyLayerSchemaNestedObject(),
			...
			NestedObject: schema.NestedAttributeObject{
				Attributes: map[string]schema.Attribute{
					"axis": schema.StringAttribute{
						MarkdownDescription: "Which axis the reference line applies to. Not settable against Kibana 9.6: the schema accepts `bottom`/`left`/`right`, all of which Kibana 9.6 rejects, while the values it accepts (`y`, `y2`) are not in the schema. Omit this attribute; it defaults to the Y axis. Tracked in https://github.com/elastic/terraform-provider-elasticstack/issues/4958.",
						Optional:            true,
						Validators: []validator.String{
