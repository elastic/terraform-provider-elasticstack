...
func populateLensVisByValueFromTypedChartAPI(
	...
	if visType == "" {
		if unknownTypeAddsError {
			diags.AddError(
				...
				"The lens-dashboard-app panel config has a top-level chart discriminator but could not resolve a Lens chart kind from the union; use `by_value.config_json` until this shape is modeled.",
			)
		}
		return diags
	}
	...
	if conv == nil {
		diags.AddError(
			"Unsupported visualization chart type",
			fmt.Sprintf(
				"The dashboard returned Lens visualization discriminator %q which this provider does not support as typed `lens_dashboard_app_config.by_value`. "+
					"Use `by_value.config_json` as the escape hatch to manage this panel until support is added.",
				visType,
			),
		)
		return diags
