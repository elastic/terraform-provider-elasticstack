...
func readSloBurnRateDrilldownsFromAPI(
	...
	items := make([]panelkit.URLDrilldownAPIItemData, 0, len(*apiDrilldowns))
	for _, item := range *apiDrilldowns {
		d, err := item.AsKibanaHTTPAPIsSloBurnRateEmbeddableDrilldowns0()
		if err != nil {
			continue
		}
