...
func Test_imagePanelToAPI_fileSrc(t *testing.T) {
	...
	item, diags := imagePanelToAPI(pm, grid, nil)
...
func Test_populateImagePanelFromAPI_nullPreservation(t *testing.T) {
	...
	pm := prior
	populateImagePanelFromAPI(&pm, &prior, apiPanel)
...
func Test_imagePanelDrilldownEntryValidator(t *testing.T) {
	ctx := context.Background()
	v := imagePanelDrilldownEntryValidator{}
