...
func TestOsqueryPackModel_satisfiesKibanaResourceModel(t *testing.T) {
	t.Parallel()
	var _ entitycore.KibanaResourceModel = osqueryPackModel{}
}

func TestOsqueryPackModel_satisfiesWithVersionRequirements(t *testing.T) {
	t.Parallel()
...
func TestResource_embedsEntityCoreKibanaResource(t *testing.T) {
	...
	require.True(t, ok, "Resource should embed *entitycore.KibanaResource[osqueryPackModel] as field KibanaResource")
	require.True(t, field.Anonymous)
	require.Equal(t, reflect.TypeFor[*entitycore.KibanaResource[osqueryPackModel]](), field.Type)
}
...
func TestResource_importState_passthroughCompoundID(t *testing.T) {
	...
	const importID = "production/pack-uuid-123"
