...
func (r *exceptionContainerResource) Create(ctx context.Context, request resource.CreateRequest, response *resource.CreateResponse) {
	...
	resp, err := oapiClient.API.CreateExceptionListWithResponse(ctx, apiModel)
	if err != nil {
	...
	case http.StatusOK:
		...
		exists, diags := r.readExceptionListFromAPI(ctx, client, &plan)
		response.Diagnostics.Append(diags...)
		if response.Diagnostics.HasError() {
			return
		}
		...
		if !exists {
			response.Diagnostics.AddError("Exception list not found after creation", "The exception list was created but could not be found afterward")
			return
		}
	default:
