...
func (m *Model) toAPICreateModel(ctx context.Context) (*kbapi.CreateListItemJSONRequestBody, diag.Diagnostics) {
	...
	if typeutils.IsKnown(m.ListItemID) {
...
func (m *Model) fromAPIModel(ctx context.Context, apiItem *kbapi.SecurityListsAPIListItem) diag.Diagnostics {
	...
	m.Meta = typeutils.MarshalJSONToNormalized(apiItem.Meta, &diags)
	if diags.HasError() {
		return diags
	}
