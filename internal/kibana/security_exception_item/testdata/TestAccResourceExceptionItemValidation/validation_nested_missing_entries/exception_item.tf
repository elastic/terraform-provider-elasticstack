...
resource "elasticstack_kibana_security_exception_item" "test" {
  ...
  item_id        = var.item_id
  name           = "Test Exception Item - Nested Missing Entries"
  description    = "Test validation: nested entry without entries"
  type           = "simple"
  namespace_type = "single"
  entries = [
    {
      type  = "nested"
      field = "parent.field"
      # Missing entries - should trigger validation error
    }
  ]
}
