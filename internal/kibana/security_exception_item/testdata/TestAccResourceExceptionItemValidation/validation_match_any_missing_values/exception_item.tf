...
resource "elasticstack_kibana_security_exception_item" "test" {
  ...
  item_id        = var.item_id
  name           = "Test Exception Item - MatchAny Missing Values"
  description    = "Test validation: match_any entry without values"
  type           = "simple"
  namespace_type = "single"
  entries = [
    {
      ...
      field    = "process.name"
      operator = "included"
      # Missing values - should trigger validation error
    }
  ]
}
