		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("elasticstack_elasticsearch_transform.test_advanced", "source.runtime_mappings",
						`{"day_of_week":{"script":{"source":"emit(doc['order_date'].value.dayOfWeekEnum.getDisplayName(TextStyle.FULL, Locale.ROOT))"},"type":"keyword"}}`),
