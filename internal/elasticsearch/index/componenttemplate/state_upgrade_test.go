...
func runUpgradeV1(t *testing.T, raw map[string]any) *resource.UpgradeStateResponse {
	t.Helper()
	rawJSON, err := json.Marshal(raw)
	require.NoError(t, err)
	...
	upgraders := r.UpgradeState(context.Background())
	up, ok := upgraders[1]
	require.True(t, ok, "missing v1 upgrader")
...
// baseV1ComponentTemplateState returns a realistic v1 state shape (object-typed
// template block; no data_stream_options key) for v1→v2 upgrade tests.
func baseV1ComponentTemplateState() map[string]any {
	return map[string]any{
		"id":   "cluster-uuid/my-template",
		"name": "my-template",
		"elasticsearch_connection": []any{
...
func TestComponentTemplateUpgradeStateV1ToV2_injects_null_data_stream_options(t *testing.T) {
	t.Parallel()
	raw := baseV1ComponentTemplateState()
	resp := runUpgradeV1(t, raw)
	require.False(t, resp.Diagnostics.HasError(), "%s", resp.Diagnostics)
	...
	tmpl, ok := got["template"].(map[string]any)
	require.True(t, ok)
	v, ok := tmpl["data_stream_options"]
	require.True(t, ok, "expected data_stream_options key to be injected")
	require.Nil(t, v, "expected data_stream_options to be null")

	requireUpgradedStateDecodes(t, resp)
}

func TestComponentTemplateUpgradeStateV1ToV2_template_null_noop(t *testing.T) {
	t.Parallel()
	raw := baseV1ComponentTemplateState()
	raw["template"] = nil
...
func TestComponentTemplateUpgradeStateV1ToV2_invalid_json(t *testing.T) {
	t.Parallel()

	r := newResource()
	up := r.UpgradeState(context.Background())[1]
	resp := &resource.UpgradeStateResponse{}
	up.StateUpgrader(context.Background(), resource.UpgradeStateRequest{
		RawState: &tfprotov6.RawState{JSON: []byte(`{`)},
	}, resp)
