...
// ValidateRestrictionSupport checks the live cluster version against the
// `restriction` blocks declared in `role_descriptors` and emits a diagnostic
// when the server is too old to accept them. Called from both the create and
// update flows.
