...
const (
	...
	OwnerDescription               = "Whether the API key being deleted is owned by the current authenticated user. Defaults to `true`, which allows deletion under the `manage_own_api_key` cluster privilege. Set to `false` only if the connection's user is expected to delete API keys it does not own, which requires the `manage_api_key` cluster privilege instead."
