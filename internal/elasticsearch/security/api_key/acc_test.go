		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("elasticstack_elasticsearch_security_api_key.test", "access.search.0.field_security", `{"grant":["title","body"]}`),
					resource.TestCheckResourceAttr("elasticstack_elasticsearch_security_api_key.test", "access.search.0.query", `{"match":{"status":"published"}}`),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("elasticstack_elasticsearch_security_api_key.test", "access.search.0.names.1", "metrics-*"),
					resource.TestCheckResourceAttr("elasticstack_elasticsearch_security_api_key.test", "access.search.0.field_security", `{"grant":["title","body","tags"]}`),
					resource.TestCheckResourceAttr("elasticstack_elasticsearch_security_api_key.test", "access.search.0.query", `{"match":{"status":"active"}}`),
