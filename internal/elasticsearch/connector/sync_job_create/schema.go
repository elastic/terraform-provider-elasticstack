...
const schemaMarkdownDescription = `Creates an Elasticsearch connector sync job on demand. **Requires Terraform 1.14+** (provider-defined actions) and **Elasticsearch 8.16.0+** ` +
	`(the on-wire body shape stabilized in 8.16; the underlying API is available from 8.12 but rejects the field name the typed client produces).
