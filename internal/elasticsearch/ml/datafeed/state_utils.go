...
func WaitForDatafeedState(ctx context.Context, client *clients.ElasticsearchScopedClient, datafeedID string, desiredState State) (bool, diag.Diagnostics) {
	return ml.WaitForResourceState(ctx, "datafeed", datafeedID, ml.WaitForResourceStateConfig[State]{
		Get: func(ctx context.Context) (*State, diag.Diagnostics) {
			return GetDatafeedState(ctx, client, datafeedID)
		},
		Desired:  desiredState,
		Terminal: terminalDatafeedStates,
	})
}
