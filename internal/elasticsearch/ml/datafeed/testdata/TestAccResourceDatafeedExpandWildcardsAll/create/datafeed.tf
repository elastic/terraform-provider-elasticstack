...
resource "elasticstack_elasticsearch_ml_anomaly_detection_job" "test" {
  job_id      = var.job_id
  description = "Test job for expand_wildcards all normalization"
...
resource "elasticstack_elasticsearch_ml_datafeed" "test" {
  ...
  # Use "all" — the API expands this to ["open","closed","hidden"].
  # The custom ExpandWildcardsType semantic equality must suppress the
  # resulting perpetual diff on subsequent plans.
  indices_options = {
