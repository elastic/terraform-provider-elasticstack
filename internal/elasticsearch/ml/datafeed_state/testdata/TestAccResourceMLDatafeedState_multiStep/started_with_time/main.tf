...
resource "elasticstack_elasticsearch_ml_datafeed_state" "nginx" {
  ...
  state       = "started"
  // Explicitly setting a non-standard timezone here. 
  // The RFC3339 type compares times including their timezone info.
  // The resource must preserve the user supplied timezone info when reading from the API in order 
  // to avoid an inconsistent state error.
