...
resource "elasticstack_elasticsearch_ml_anomaly_detection_job" "test" {
  job_id      = var.job_id
  description = "Test job for datafeed state multi-step time testing"
  analysis_config = {
    bucket_span = "1h"
    detectors = [{
      function             = "count"
...
# end is deliberately omitted to exercise the unset-end transition.
# start and end are both absent; the datafeed is stopped, verifying that
# end does not linger in state after being removed from config.
