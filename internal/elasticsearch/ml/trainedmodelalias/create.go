...
func createTrainedModelAlias(ctx context.Context, client *clients.ElasticsearchScopedClient, req entitycore.WriteRequest[TFModel]) (entitycore.WriteResult[TFModel], fwdiags.Diagnostics) {
	...
	tflog.Debug(ctx, fmt.Sprintf("Creating ML trained model alias: %s", alias))
	...
	compID, idDiags := client.ID(ctx, alias)
	...
	tflog.Debug(ctx, fmt.Sprintf("Successfully created ML trained model alias: %s", alias))
	return entitycore.WriteResult[TFModel]{Model: plan}, diags
}
