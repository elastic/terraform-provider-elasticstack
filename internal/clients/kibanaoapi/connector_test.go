...
func Test_connectorResponseToModel(t *testing.T) {
		...
		},
		{
			...
			response: &kbapi.ConnectorResponse{
				...
				Config: func() *map[string]*any {
					webhookURL := any("https://hooks.slack.com/services/xxx")
					m := map[string]*any{"webhookUrl": &webhookURL}
					return &m
				}(),
			},
			expectedModel: &models.KibanaActionConnector{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			model, err := kibanaoapi.ConnectorResponseToModel(tt.spaceID, tt.response)
...
func TestConnectorConfigWithDefaults(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := kibanaoapi.ConnectorConfigWithDefaults(tt.connectorTypeID, tt.planConfig)
