...
func Test_actionConnectorToModel(t *testing.T) {
	now := time.Now()
	tests := []struct {
		name          string
		spaceId       string
		connectorResponse  *connectors.ConnectorResponseProperties
		expectedModel *models.KibanaActionConnector
	}{
		{
			name:          "nil response should return a nil model",
			spaceId:       "space-id",
			connectorResponse:  nil,
			expectedModel: nil,
		},
		{
		...
		},
		{
			...
			spaceId: "space-id",
			connectorResponse: &alerting.RuleResponseProperties{
				Id:         "id",
				Name:       "name",
				Consumer:   "consumer",
				Params:     map[string]interface{}{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			model := ruleResponseToModel(tt.spaceId, tt.connectorResponse)
