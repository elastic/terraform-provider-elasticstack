...
// upgradeStateV0 migrates legacy schema version 0 state (where the ssl attribute was stored as a list)
// into schema version 1 (ssl as a single nested object).
func upgradeStateV0(_ context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
	...
	resp.Diagnostics.Append(stateutil.CollapseListPath(stateMap, "ssl", "ssl")...)
	if resp.Diagnostics.HasError() {
		return
	}
