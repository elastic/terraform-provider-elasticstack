...
func TestResource_embedsKibanaResource(t *testing.T) {
	t.Parallel()
	rt := reflect.TypeFor[outputResource]()
	field, ok := rt.FieldByName("KibanaResource")
	require.True(t, ok)
	require.True(t, field.Anonymous)
	require.Equal(t, reflect.TypeFor[*entitycore.KibanaResource[outputModel]](), field.Type)
}

// Import splits "<space>/<output_id>" into structured attributes instead of
// preserving the raw import string on id (passthrough shape).
func TestResource_importState_customCompositeID(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	r, ok := any(newOutputResource()).(resource.ResourceWithImportState)
	require.True(t, ok)
	st := providerfwtest.EmptyImportState(t, r)
	resp := &resource.ImportStateResponse{State: st}

	r.ImportState(ctx, resource.ImportStateRequest{ID: "myspace/the-output-id"}, resp)
	require.False(t, resp.Diagnostics.HasError())

	var outputID types.String
	resp.Diagnostics.Append(resp.State.GetAttribute(ctx, path.Root("output_id"), &outputID)...)
	require.False(t, resp.Diagnostics.HasError())
	require.Equal(t, "the-output-id", outputID.ValueString())
