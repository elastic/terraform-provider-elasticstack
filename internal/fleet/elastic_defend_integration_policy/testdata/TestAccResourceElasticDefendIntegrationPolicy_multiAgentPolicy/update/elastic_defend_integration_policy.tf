...
resource "elasticstack_fleet_agent_policy" "three" {
  name      = "${var.policy_name}-agent-policy-three"
  namespace = "default"
}
...
resource "elasticstack_fleet_elastic_defend_integration_policy" "test" {
  ...
  namespace           = "default"
  agent_policy_ids    = [elasticstack_fleet_agent_policy.one.policy_id, elasticstack_fleet_agent_policy.three.policy_id]
  enabled             = true
