...
func (model *integrationPolicyModel) populateInputFromAPI(ctx context.Context, inputs map[string]kbapi.PackagePolicyInput, diags *diag.Diagnostics) {
	// Only populate inputs if they were originally configured
	// This avoids Terraform detecting inconsistent state when inputs are auto-generated by the API
	originallyConfiguredInputs := utils.IsKnown(model.Input) && !model.Input.IsNull()

	if !originallyConfiguredInputs {
		// Inputs were not configured, keep as empty to avoid inconsistent state
