...
variable "additional_datastreams_permissions" {
  description = "Data streams the integration's API key may write to beyond its own"
  type        = list(string)
}
...
resource "elasticstack_fleet_integration_policy" "test_policy" {
  ...
  namespace           = "default"
  description         = "IntegrationPolicyTest Policy with reroute permissions"
  ...
  additional_datastreams_permissions = var.additional_datastreams_permissions
