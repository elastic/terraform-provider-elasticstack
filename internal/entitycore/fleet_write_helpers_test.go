...
func TestSimpleFleetUpdate(t *testing.T) {
	t.Run("happy path forwards writeID to toBody and spaceID/writeID to apiUpdate then populate", func(t *testing.T) {
		...
		writeFn := SimpleFleetUpdate[simpleWriteTestModel, string, string](
			...
			func(_ context.Context, _ *fleetclient.Client, spaceID, writeID string, _ string) (*string, diag.Diagnostics) {
				gotSpaceID = spaceID
				gotWriteID = writeID
				return &resp, nil
			},
	...
	t.Run("prior spaceID overrides req.SpaceID for both toBody's caller and apiUpdate", func(t *testing.T) {
		var gotSpaceID string

		writeFn := SimpleFleetUpdate[simpleWriteTestModel, string, string](
			func(plan simpleWriteTestModel, _ context.Context, _ string) (string, diag.Diagnostics) {
				return "body", nil
			},
			func(_ context.Context, _ *fleetclient.Client, spaceID, _ string, _ string) (*string, diag.Diagnostics) {
				gotSpaceID = spaceID
				resp := "updated-body"
				return &resp, nil
			},
