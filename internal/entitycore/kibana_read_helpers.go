...
// SimpleKibanaRead returns a [KibanaReadFunc] for the common
// fetch-then-populate shape shared by simple Kibana resources: call apiGet for
...
func SimpleKibanaRead[T KibanaResourceModel, R any](
	...
	return func(ctx context.Context, client *clients.KibanaScopedClient, resourceID string, spaceID string, prior T) (T, bool, diag.Diagnostics) {
		...
		data, d := apiGet(ctx, client.GetKibanaOapiClient(), spaceID, resourceID)
		diags.Append(d...)
		if diags.HasError() {
