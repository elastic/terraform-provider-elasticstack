...
// NOTE: This test validated the old agentic workflow pattern where PR evidence
// was gathered via a separate github-script step with explicit env var passing.
// The changelog-generation workflow now uses a deterministic Go engine invoked
// via `go run ./scripts/changelog-generation/...` instead. The test is skipped
// to avoid false failures against the legacy workflow.md.tmpl.
test.skip('workflow passes release context into gather-pr-evidence via supported env vars (superseded by Go engine)', () => {
  // Previously validated the Gather PR evidence step env var contract.
  // The new workflow.yml.tmpl invokes the Go engine directly with no separate
