...
func TestValidateMode_invalid(t *testing.T) {
	...
	if err == nil || err.Error() != `invalid changelog mode: "staging" (must be 'unreleased' or 'release')` {
		t.Fatalf("got %v", err)
	}
}
...
func TestValidateMode_releaseMissingTarget(t *testing.T) {
	...
	want := "release mode requires a non-empty semver target version X.Y.Z without a leading 'v'"
...
func TestValidateMode_releaseLeadingVRejected(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "leading 'v'") {
		t.Fatalf("got %v", err)
	}
}
