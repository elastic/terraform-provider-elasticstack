...
- Add `elasticstack_fleet_managed_integration` resource for managing Fleet managed integrations (Elastic Cloud Hosted / Serverless, Kibana 9.5.0+). Same-core Kibana `-SNAPSHOT` builds satisfy the version floor via the provider-wide Kibana `EnforceMinVersion` convention (Elasticsearch uses strict semver only). ([#4034](https://github.com/elastic/terraform-provider-elasticstack/pull/4034))
