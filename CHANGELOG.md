...
- Add optional `calendars` on `elasticstack_elasticsearch_ml_anomaly_detection_job` to assign the job to ML calendars (via `_ml/calendars/{calendar_id}/jobs/{job_id}`).
