...
- Add `elasticstack_elasticsearch_query_ruleset` resource and `data.elasticstack_elasticsearch_query_ruleset` data source for managing Elasticsearch Query Rules API rulesets (pinned and exclude rules with full criteria/actions semantics, including JSON-encoded numeric values, import by composite `<cluster_uuid>/<ruleset_id>` ID, and a minimum cluster version guard of 8.12.0). ([#1064](https://github.com/elastic/terraform-provider-elasticstack/issues/1064))
