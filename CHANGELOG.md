...
- Add `elasticstack_elasticsearch_query_ruleset` resource and `data.elasticstack_elasticsearch_query_ruleset` data source for managing Elasticsearch Query Rules API rulesets (pinned and exclude rules with full criteria/actions semantics, including JSON-encoded numeric values, import by composite `<cluster_uuid>/<ruleset_id>` ID, and a minimum cluster version guard of 8.16.0). ([#1064](https://github.com/elastic/terraform-provider-elasticstack/issues/1064))
...
- Fix `Provider produced inconsistent result after apply: .config_yaml: inconsistent values for sensitive attribute` on `elasticstack_fleet_output` updates. The Fleet API treats an omitted `config_yaml` in a PUT as "no change" and echoes the previously stored value (or an empty string for outputs that never had one) back in the response; the shared reader now normalises empty `config_yaml` from the API to null state and preserves a user-removed null across the API echo on both update and refresh so removing the attribute applies cleanly and stays clean. Import continues to populate `config_yaml` from the API. `config_yaml` also adopts `customtypes.NormalizedYamlType`, so semantically equivalent YAML (whitespace, key order) no longer triggers spurious updates. ([#1856](https://github.com/elastic/terraform-provider-elasticstack/issues/1856))
- `elasticstack_kibana_import_saved_objects` now surfaces the Kibana `message` field as the diagnostic detail when the Saved Objects Import API returns a non-200, non-400 HTTP error (e.g. HTTP 422 for a saved object exported from a more recent Kibana version), instead of showing the raw JSON response body. Falls back to the previous raw-body behavior when the response is not a valid Kibana Boom envelope.
