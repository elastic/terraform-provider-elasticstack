...
#### Scenario: Release PR branch matches manual release-mode contract

- **WHEN** the release preparation workflow creates a release branch
- **THEN** that branch name SHALL match the `prep-release-*` naming pattern expected by the changelog generator for explicit release-mode checkout
