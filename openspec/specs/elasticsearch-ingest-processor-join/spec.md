# `elasticstack_elasticsearch_ingest_processor_join` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest/processor_join_data_source.go`

## Purpose

Define the Terraform schema and runtime behavior for the `elasticstack_elasticsearch_ingest_processor_join` data source. This data-only source accepts Elasticsearch join ingest processor configuration and produces a serialized JSON representation suitable for inclusion in an ingest pipeline definition. No Elasticsearch API calls are made.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_join" "example" {
  # Processor-specific (required)
  field     = <required, string>  # field containing array values to join
  separator = <required, string>  # separator character used to join array elements
...
The data source SHALL serialize the processor configuration into a JSON object wrapped under the key `"join"`, using `json.MarshalIndent` with a single-space indent. The resulting string SHALL be stored in the `json` computed attribute.

#### Scenario: JSON wrapping

- GIVEN a configuration with `field = "joined_array"` and `separator = "-"`
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object of the form `{"join": {...}}`
...
The data source SHALL require `field` (string) and `separator` (string) to be set. Omitting either SHALL cause a validation error before the read function runs.

#### Scenario: Missing required field

- GIVEN a configuration that omits `field` or `separator`
- WHEN Terraform validates the configuration
- THEN a validation error SHALL be returned

### Requirement: Optional processor-specific attributes (REQ-005)

The data source SHALL include the following optional processor-specific attribute in the JSON output when set, and omit it when not set:
