...
The job SHALL check out the repository using `actions/checkout` pinned by commit SHA. The job SHALL install Go using `actions/setup-go` with `go-version-file: go.mod` and Go module caching enabled. The job SHALL install Terraform using `hashicorp/setup-terraform` with `terraform_wrapper: false`.
