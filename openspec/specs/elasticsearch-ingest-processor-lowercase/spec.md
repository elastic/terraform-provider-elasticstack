# `elasticstack_elasticsearch_ingest_processor_lowercase` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest/processor_lowercase_data_source.go`

## Purpose

Define the Terraform schema and runtime behavior for the `elasticstack_elasticsearch_ingest_processor_lowercase` data source. This data-only source accepts Elasticsearch lowercase ingest processor configuration and produces a serialized JSON representation suitable for inclusion in an ingest pipeline definition. No Elasticsearch API calls are made.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_lowercase" "example" {
  # Processor-specific (required)
  field = <required, string>  # field to convert to lowercase
...
The data source SHALL serialize the processor configuration into a JSON object wrapped under the key `"lowercase"`, using `json.MarshalIndent` with a single-space indent. The resulting string SHALL be stored in the `json` computed attribute.

#### Scenario: JSON wrapping

- GIVEN a configuration with `field = "foo"`
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object of the form `{"lowercase": {...}}`
