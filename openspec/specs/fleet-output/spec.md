...
Define schema and behavior for the Fleet output resource and data source. The resource manages Fleet outputs (Elasticsearch, remote Elasticsearch, Logstash, and Kafka) via the Fleet Outputs API, including Kibana space awareness. The data source lists all outputs available in a given Kibana space.
...
resource "elasticstack_fleet_output" "example" {
  ...
  type  = <required, string>             # one of: "elasticsearch", "remote_elasticsearch", "logstash", "kafka"
...
- GIVEN Fleet returns outputs of types elasticsearch, remote_elasticsearch, logstash, and kafka
- WHEN the data source read runs
