# `elasticstack_elasticsearch_ingest_processor_circle` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest/processor_circle_data_source.go`

## Purpose

Provide a data-only source that serializes an Elasticsearch circle ingest processor configuration to JSON. The circle processor converts circle definitions expressed in WKT or GeoJSON format into regular polygons of an approximate equivalent area. No API calls are made; all computation is local.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_circle" "example" {
  field          = <required, string>  # The field containing the circle definition
  error_distance = <required, float>   # Allowed difference between inscribed polygon and circle radius (metres for geo_shape, unit-less for shape)
  shape_type     = <required, string>  # Field mapping type: "geo_shape" or "shape"
...
The data source SHALL serialize the processor configuration as `{"circle": {...}}` JSON and store the result in the `json` computed attribute.

#### Scenario: JSON output wraps processor type

- GIVEN valid configuration is applied
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object with a single top-level key `"circle"` whose value includes all configured fields
...
The data source SHALL require `field` (string), `error_distance` (float), and `shape_type` (string). If any is absent the provider SHALL return a validation error.

#### Scenario: Missing required field

- GIVEN `field`, `error_distance`, or `shape_type` is not configured
- WHEN Terraform plans or applies
- THEN the provider SHALL return a validation error

### Requirement: shape_type validation (REQ-005)

`shape_type` SHALL be validated to accept only `"geo_shape"` or `"shape"`. Any other value SHALL cause the provider to return a validation error.

#### Scenario: Invalid shape_type

- GIVEN `shape_type` is set to a value other than `"geo_shape"` or `"shape"`
- WHEN Terraform plans or applies
- THEN the provider SHALL return a validation error
