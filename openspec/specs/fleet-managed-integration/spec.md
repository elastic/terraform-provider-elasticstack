# fleet-managed-integration Specification

## Purpose

Define the schema and runtime behaviour of the `elasticstack_fleet_managed_integration` resource (`internal/fleet/managedintegration`), including full CRUD against the Kibana Fleet `managed_integrations` API, in-place name and package version updates, full-replace PUT semantics, and `global_data_tags` MapNestedAttribute modelling. Supersedes the pre-release `elasticstack_fleet_agentless_policy` resource, which is removed from the provider without a deprecation shim.
