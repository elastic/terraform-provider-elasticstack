# `elasticstack_elasticsearch_ingest_processor_foreach` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest`

## Purpose

Define the schema and runtime behavior for the `elasticstack_elasticsearch_ingest_processor_foreach` data source, which builds an Elasticsearch foreach ingest processor configuration and serializes it to JSON. This data source performs no API calls; it computes a JSON representation and a deterministic hash ID from the configured processor parameters.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_foreach" "example" {
  field          = <required, string>  # Field containing array or object values to iterate over
  processor      = <required, string>  # JSON string; the ingest processor to run on each element
...
The data source SHALL serialize the processor configuration to a JSON string stored in `json`. The JSON SHALL be wrapped in a top-level `"foreach"` key, producing the shape `{"foreach": {...}}`.

#### Scenario: JSON wrapping

- GIVEN a valid configuration
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object with a top-level `"foreach"` key whose value is the processor parameters
...
The data source SHALL require `field` and `processor`. If either is absent, Terraform SHALL report a configuration error before the read function is called.

#### Scenario: Missing required attribute

- GIVEN a configuration that omits `field` or `processor`
- WHEN Terraform validates the configuration
- THEN a configuration error SHALL be raised

### Requirement: processor JSON parsing (REQ-005)

The `processor` attribute SHALL be a valid JSON string. The data source SHALL parse it as a JSON object and include it as the `processor` value in the serialized output. If `processor` is not valid JSON, the data source SHALL return an error diagnostic.

#### Scenario: Invalid processor JSON

- GIVEN `processor` contains invalid JSON
- WHEN the data source is read
- THEN the data source SHALL return an error diagnostic and SHALL NOT set `json`

### Requirement: Default attribute values (REQ-006)

When `ignore_missing` is not configured, the data source SHALL use `false`. When `ignore_failure` is not configured, the data source SHALL use `false`.
