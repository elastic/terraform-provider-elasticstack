# `elasticstack_elasticsearch_ingest_processor_dissect` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest`

## Purpose

Define the schema and runtime behavior for the `elasticstack_elasticsearch_ingest_processor_dissect` data source, which builds an Elasticsearch dissect ingest processor configuration and serializes it to JSON. This data source performs no API calls; it computes a JSON representation and a deterministic hash ID from the configured processor parameters.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_dissect" "example" {
  field            = <required, string>    # The field to dissect
  pattern          = <required, string>    # The dissect pattern to apply
  append_separator = <optional, string>    # Separator for appended fields; default ""
...
The data source SHALL serialize the processor configuration to a JSON string stored in `json`. The JSON SHALL be wrapped in a top-level `"dissect"` key, producing the shape `{"dissect": {...}}`.

#### Scenario: JSON wrapping

- GIVEN a valid configuration
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object with a top-level `"dissect"` key whose value is the processor parameters
...
The data source SHALL require `field` and `pattern`. If either is absent, Terraform SHALL report a configuration error before the read function is called.

#### Scenario: Missing required attribute

- GIVEN a configuration that omits `field` or `pattern`
- WHEN Terraform validates the configuration
- THEN a configuration error SHALL be raised

### Requirement: Default attribute values (REQ-005)

When `append_separator` is not configured, the data source SHALL use an empty string (`""`) as its value. When `ignore_missing` is not configured, the data source SHALL use `false`. When `ignore_failure` is not configured, the data source SHALL use `false`.

#### Scenario: Defaults in JSON output

- GIVEN a configuration that sets only the required attributes
- WHEN the data source is read
- THEN `json` SHALL include `"append_separator": ""`, `"ignore_missing": false`, and `"ignore_failure": false`

### Requirement: Optional common fields (REQ-006)
...
### Requirement: on_failure JSON parsing (REQ-007)
