...
- **WHEN** the `validate-pr-section` subcommand of `scripts/changelog/` and `run-engine`, which assembles per-PR changelog content from merged PR bodies, are exercised on the same `## Changelog` input shape
