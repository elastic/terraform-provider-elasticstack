# `elasticstack_elasticsearch_ingest_processor_pipeline` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest/processor_pipeline_data_source.go`

## Purpose

Provide a data-only source that serializes an Elasticsearch pipeline ingest processor configuration to JSON. The pipeline processor executes another ingest pipeline, allowing pipelines to be composed. No API calls are made; all computation is local.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_pipeline" "example" {
  name = <required, string> # The name of the pipeline to execute
...
The data source SHALL serialize the processor configuration as `{"pipeline": {...}}` JSON and store the result in the `json` computed attribute.

#### Scenario: JSON output wraps processor type

- GIVEN valid configuration is applied
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object with a single top-level key `"pipeline"` whose value includes all configured fields
...
The data source SHALL require `name` (string). If `name` is absent the provider SHALL return a validation error.

#### Scenario: Missing required field

- GIVEN `name` is not configured
