# `elasticstack_elasticsearch_ingest_processor_split` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest/processor_split_data_source.go`

## Purpose

Provide a data-only source that serializes an Elasticsearch split ingest processor configuration to JSON. The split processor splits a string field into an array of substrings using a regular expression separator. No API calls are made; all computation is local.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_split" "example" {
  field             = <required, string>  # The field to split
  separator         = <required, string>  # Regex matching the separator (e.g. "," or "\\s+")
  target_field      = <optional, string>  # Output field; defaults to updating field in-place
  preserve_trailing = <optional, bool>    # Preserve empty trailing fields. Default: false
...
The data source SHALL serialize the processor configuration as `{"split": {...}}` JSON and store the result in the `json` computed attribute.

#### Scenario: JSON output wraps processor type

- GIVEN valid configuration is applied
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object with a single top-level key `"split"` whose value includes all configured fields
...
The data source SHALL require `field` (string) and `separator` (string). If either is absent the provider SHALL return a validation error.

#### Scenario: Missing required field

- GIVEN `field` or `separator` is not configured
- WHEN Terraform plans or applies
- THEN the provider SHALL return a validation error

### Requirement: preserve_trailing and ignore_missing defaults (REQ-005)

When `preserve_trailing` is not configured, the data source SHALL default to `false` and include `"preserve_trailing": false` in the serialized JSON. When `ignore_missing` is not configured, the data source SHALL default to `false` and include `"ignore_missing": false` in the serialized JSON.

#### Scenario: Default boolean values

- GIVEN `preserve_trailing` and `ignore_missing` are not set
- WHEN the data source is read
- THEN the serialized JSON SHALL include `"preserve_trailing": false` and `"ignore_missing": false`

### Requirement: Optional target_field (REQ-006)

When `target_field` is configured, the data source SHALL include it in the serialized JSON. When `target_field` is not configured, the data source SHALL omit it from the serialized JSON, meaning the split result is written back to `field` in-place.
