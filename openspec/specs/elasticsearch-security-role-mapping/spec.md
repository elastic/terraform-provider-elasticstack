...
On read, the resource and data source SHALL serialize `rules` from the API response into a normalized JSON string and store it in state. The read path SHALL preserve the raw JSON shape produced by the typed API client, including array form for single-element field values (no single-element-array collapsing). Semantic equality (REQ-023) handles any string-vs-array mismatch transparently during plan comparison.

On read, when `role_templates` is present and non-empty in the API response, the resource and data source SHALL serialize it as a normalized JSON array string and store it in state.
