# `elasticstack_elasticsearch_ingest_processor_rename` — Schema and Functional Requirements

Data source implementation: `internal/elasticsearch/ingest/processor_rename_data_source.go`

## Purpose

Provide a data-only source that serializes an Elasticsearch rename ingest processor configuration to JSON. The rename processor renames an existing field to a new name. No API calls are made; all computation is local.

## Schema

```hcl
data "elasticstack_elasticsearch_ingest_processor_rename" "example" {
  field          = <required, string> # The field to be renamed
  target_field   = <required, string> # The new name of the field
...
The data source SHALL serialize the processor configuration as `{"rename": {...}}` JSON and store the result in the `json` computed attribute.

#### Scenario: JSON output wraps processor type

- GIVEN valid configuration is applied
- WHEN the data source is read
- THEN `json` SHALL contain a JSON object with a single top-level key `"rename"` whose value includes all configured fields
...
The data source SHALL require both `field` (string) and `target_field` (string). If either is absent the provider SHALL return a validation error.

#### Scenario: Missing required field

- GIVEN `field` or `target_field` is not configured
