...
The resource SHALL manage private locations through Kibana's Synthetics Private Locations API: create via the legacy Kibana client's `KibanaSynthetics.PrivateLocation.Create`, read via `KibanaSynthetics.PrivateLocation.Get`, and delete via `KibanaSynthetics.PrivateLocation.Delete`.

#### Scenario: CRUD uses Private Locations API

- GIVEN a managed Synthetics private location
- WHEN create, read, or delete runs
- THEN the provider SHALL use the corresponding Kibana Synthetics Private Location API operation
...
- THEN the provider SHALL extract `<resource_id>` and use it to call the Synthetics Private Location API
...
Changes to `id`, `label`, `agent_policy_id`, or `tags` SHALL each require resource replacement rather than an in-place update. The `geo` block does not carry `RequiresReplace` independently but changes to it in practice trigger replacement through the interaction with REQ-006 (update not supported).
