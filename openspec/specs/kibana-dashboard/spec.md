...
Schema validation SHALL enforce that each typed panel config block is only present on a panel whose `type` matches that block's panel type, and that at most one typed config block is present on any panel. This exclusivity requirement applies to `time_slider_control_config`, `slo_burn_rate_config`, and `slo_error_budget_config` in addition to all previously supported typed config blocks.
