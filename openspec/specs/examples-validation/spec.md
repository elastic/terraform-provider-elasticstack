# `examples-validation` — Example Validation Requirements

Implementation: [`examples/examples.go`](../../../examples/examples.go), [`internal/acctest/examples_plan_test.go`](../../../internal/acctest/examples_plan_test.go)

## Purpose

Ensure Terraform examples under `examples/resources/` and `examples/data-sources/` remain copy-pasteable at plan time by running each covered `.tf` file in isolation through the provider acceptance-test harness.
