# changelog-tooling Specification

## Purpose

Defines the Go-based changelog tooling at `scripts/changelog/` used by changelog-generation and PR changelog-check workflows. This capability superseded JavaScript helpers under `.github/scripts/workflows/` (see OpenSpec change `migrate-changelog-engine-to-go`).

## Requirements
### Requirement: Single Go CLI under `scripts/changelog/`

The repository SHALL provide a Go command-line tool at `scripts/changelog/` that subsumes all behaviour previously implemented by `.github/scripts/workflows/lib/changelog-*.js`, `.github/scripts/workflows/changelog/*.js`, `.github/scripts/workflows/lib/pr-changelog-*.js`, and `.github/scripts/workflows/pr-changelog-check/*.js`. The tool MUST be invokable as `go run ./scripts/changelog <subcommand>` from any workflow step that has previously run `actions/setup-go` and `actions/checkout`.
...
The tool SHALL implement the `## Changelog` PR-body parser exactly once, in an internal package consumed by every subcommand that needs to read PR-body changelog content (`run-engine`, which assembles changelog content from merged-PR bodies, and `validate-pr-section`). No subcommand may carry its own parallel parser implementation.
...
#### Scenario: Validator and engine parse the same PR-body section identically

- **WHEN** `validate-pr-section` receives a PR body containing a `## Changelog` section and `run-engine` assembles changelog content from a merged PR whose body contains the same `## Changelog` shape
- **THEN** both code paths derive the same customer impact, summary, and breaking-changes content via the shared `internal/section` parser
...
- `internal/prmgmt` — pull-request lifecycle helpers for changelog automation branches (`generated-changelog`, release refresh).
...
- `internal/semver` — release tag enumeration and compare-range selection.
