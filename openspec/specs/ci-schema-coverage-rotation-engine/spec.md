# ci-schema-coverage-rotation-engine Specification

## Purpose
Defines the GitHub Agentic Workflows engine configuration for the schema-coverage rotation worker, including Claude execution through the Elastic LiteLLM Anthropic-compatible proxy and an explicit per-tool timeout for repository-local commands.
