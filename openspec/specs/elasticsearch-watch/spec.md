...
When the Elasticsearch API response contains a nil (absent) `metadata` field, the resource SHALL store the JSON string `"null"` in the `metadata` state attribute, rather than the empty-object string `"{}"`. This preserves round-trip consistency for configurations that set `metadata = jsonencode(null)`.

#### Scenario: nil metadata returned by API is stored as JSON null

- **GIVEN** the configuration sets `metadata = jsonencode(null)` (the string `"null"`)
- **AND** Elasticsearch receives null metadata and returns a nil metadata field in the Get Watch
  response
...
- **AND** Elasticsearch returns a non-nil empty map for metadata
- **WHEN** read runs
