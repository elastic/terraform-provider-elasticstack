...
| HTTP client, enable/disable, response parsing, `flapping` request/response mapping, `artifacts` request/response mapping | `internal/clients/kibanaoapi/alerting_rule.go` |
| Composite id parsing | `internal/clients/api_client.go` (`CompositeID`, `CompositeIDFromStr`, `CompositeIDFromStrFw`) |
| Artifacts checksum drift handling | `modify_plan.go`, `create.go`, `update.go` |
