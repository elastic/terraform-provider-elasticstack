...
resource "elasticstack_kibana_alerting_rule" "example" {
  ...
  artifacts {                               # available in Kibana >= 8.19.0 / 9.1.0; server support validated vs stack version
    dashboards {
      id = <required, string>               # Kibana dashboard saved-object id
    }
    investigation_guide {
      content      = <optional, string>     # inline Markdown; mutually exclusive with content_path
      content_path = <optional, string>     # path to a local file; mutually exclusive with content
      checksum     = <computed, string>     # SHA-256 of file at content_path; managed by plan modifier
    }
  }
...
| `artifacts` schema, model mapping, plan modifier | `schema.go`, `models.go`, `plan_modifier.go` |
