...
- [ ] 1.2 Track [kibana#279519](https://github.com/elastic/kibana/pull/279519) to merge. Local codegen against a copy of that PR's OAS can proceed (task 3.1); committing `github_ref` in `generated/kbapi/Makefile` waits until the commit is on `elastic/kibana`.
...
- [ ] 3.1 Generate the client against the [kibana#279519](https://github.com/elastic/kibana/pull/279519) OAS. Until that PR merges, copy `oas_docs/output/kibana.yaml` from the PR branch into `generated/kbapi/oas.yaml` and run `make -C generated/kbapi transform generate`. After merge, bump `github_ref` in `generated/kbapi/Makefile` to a commit on `elastic/kibana` that includes the PR, then `make -C generated/kbapi all`.
- [ ] 3.2 Confirm the `Kibana_HTTP_APIs_alerting_*` components generate as named, reusable Go types, that `Kibana_HTTP_APIs_alerting_rule_query` produces a discriminated union with typed accessors for the composed and standalone variants, and that the diff is purely additive.

## 4. Client wrapper

- [ ] 4.1 Add `internal/clients/kibanaoapi/rule_v2.go` with create-or-replace (`PUT`), get, delete, enable, and disable functions, all returning `diag.Diagnostics`, using `kibanautil.SpaceAwarePathRequestEditor(spaceID)` for space scoping (`design.md` D6).
- [ ] 4.2 Implement request and response mapping between `internal/models` and the generated types, including the query union accessors.
...
- [ ] 4.7 Make the write path read-before-write (`design.md` D8): get the rule, seed the `PUT` body from the response, then overwrite every Terraform-modelled field unconditionally including nulls. Skip the read when the rule does not exist. `internal/fleet/agentpolicy/update.go` is the in-repo precedent.
...
- [ ] 5.9 Add `GetVersionRequirements` with the minimum version from task 1.3 (`design.md` D7).
