...
- [ ] 1.1 File the six contingent API requests from `proposal.md` against the Alerting v2 team, linking each to this change. Prioritise **1 (writable `enabled`)** and **3 (two fields named "version")** — they are the only two that would change the Terraform schema, so they must be settled before graduation ([#678](https://github.com/elastic/rna-program/issues/678)).
...
- [ ] 1.5 Determine how `alerting:v2:enabled` can be turned on in the shared CI Elastic Stack (see `dev-docs/high-level/testing.md`), since there is no Terraform resource for advanced settings. If it cannot be, decide the skip strategy for section 6 before writing tests.

## 2. Proposal review

- [ ] 2.1 Get sign-off on the resource name `elasticstack_kibana_rule` (`proposal.md`, Decision 2), explicitly resolving the "rule is an overloaded noun" objection. Update `rna-program#673` and `#655`, which currently say `elasticstack_kibana_alerting_rule_v2` / "name TBD".
...
- [ ] 5.3 Add the `metadata`, `schedule`, `grouping`, and `artifacts` blocks with their length, range, and collection-size validators (spec REQ-005, REQ-008, REQ-012).
...
- [ ] 5.12 Add `descriptions/` markdown and schema `MarkdownDescription` values. Cover: the technical-preview status, the `alerting:v2:enabled` prerequisite, the last-write-wins consequence of full-replace `PUT`, and the distinction between `metadata.version` and `version`.
...
- [ ] 6.1 Plan-only `ExpectError` tests for every cross-field rule in spec REQ-007 and for the exactly-one-of query constraint in REQ-006. These must not require a live stack (spec REQ-016).
- [ ] 6.2 Plan-only tests for the schema-level bounds in spec REQ-005 (over-long `metadata.name`, >16 `grouping.fields`, >100 `artifacts`, `state_transition.pending_count` above 1000) and for `no_data_strategy = "emit"` being rejected (REQ-014).
...
- [ ] 6.10 Gate all acceptance tests on the minimum version from task 1.3 and on `alerting:v2:enabled` being available, per the strategy chosen in task 1.5. Follow the repository's acceptance-test isolation conventions so parallel runs do not collide on rule ids or tags.
...
- [ ] 7.5 CHANGELOG entry noting the new experimental resource and the `TF_ELASTICSTACK_INCLUDE_EXPERIMENTAL` and `alerting:v2:enabled` prerequisites.
