...
- [ ] 2.2 Add `VerificationMode *string` field to `outputSSLAPIModel` struct in `models_ssl.go`
- [ ] 2.3 Populate `VerificationMode` in `objectValueToSSL()` from the Terraform object value
- [ ] 2.4 Populate `VerificationMode` in `toAPI()` when converting `outputSSLAPIModel` to `*kbapi.KibanaHTTPAPIsOutputSsl`
- [ ] 2.5 Add `verificationMode *string` parameter to `sslToObjectValue()` and use it to populate the model
- [ ] 2.6 Update the null-check guard in `sslToObjectValue()` so it remains null only when all four fields (`certificate`, `certificateAuthorities`, `key`, `verificationMode`) are nil/empty

## 3. Output Type Read Callers

- [ ] 3.1 Update `fromAPIKafkaModel()` in `models_kafka.go` to pass `data.Ssl.VerificationMode` (as `(*string)`) to `sslToObjectValue()`
