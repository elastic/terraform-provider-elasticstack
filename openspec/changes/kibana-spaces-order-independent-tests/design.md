...
- **Reuse the existing ID-scanning pattern, adding predicate variants where needed.** Use `testCheckSpaceAttrByID` for exact values, and add an ID-based predicate helper for checks such as “attribute is set” so `TestAccSpacesDataSource` preserves its existing generated-color assertion without hard-coding a value. Reuse the same lookup pattern for empty-or-absent checks rather than introducing unrelated scanning logic.
