...
- [ ] 1.1 Keep delta spec aligned with `proposal.md` / `design.md`; run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate elasticsearch-security-api-key-ephemeral --type change` (or `make check-openspec` after sync).
- [ ] 1.2 Resolve open question on whether to add a warning validator for missing `expiration` when `invalidate_on_close = false`; update delta spec if confirmed.
...
- [ ] 2.1 Add `EphemeralResources(ctx context.Context) []func() ephemeral.EphemeralResource` method to the `Provider` type in `provider/plugin_framework.go`. Register the new ephemeral resource factory. Import `"github.com/hashicorp/terraform-plugin-framework/ephemeral"`.
- [ ] 2.2 Create `internal/elasticsearch/security/api_key/ephemeral_resource.go`:
...
- [ ] 3.1 Add template `templates/ephemeral-resources/elasticstack_elasticsearch_security_api_key.md.tmpl` covering:
...
- [ ] 3.2 Run `make generate-docs` (or equivalent) to regenerate `docs/ephemeral-resources/elasticstack_elasticsearch_security_api_key.md`.

## 4. Testing

- [ ] 4.1 Add acceptance test `TestAccEphemeralResourceSecurityAPIKey` verifying:
...
- [ ] 4.2 Add acceptance test `TestAccEphemeralResourceSecurityAPIKeyInvalidateOnClose` verifying:
...
- [ ] 4.3 Add acceptance test `TestAccEphemeralResourceSecurityAPIKeyWithExpiration` verifying:
  - Setting `expiration` populates `expiration_timestamp` in the result.
- [ ] 4.4 Add acceptance test `TestAccEphemeralResourceSecurityAPIKeyCrossCluster` verifying:
...
- [ ] 4.5 Add unit tests for the ephemeral schema (attribute validation: `name` constraints, `type` one-of, `role_descriptors`/`access` mutual exclusion with type).
- [ ] 4.6 Add unit tests for `Close()` logic: confirm `DeleteAPIKey` is called when `invalidate_on_close = true` and NOT called when `false`.
