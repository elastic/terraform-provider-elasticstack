...
- **DSL plan modifier**: `data_stream_lifecycle.id` keeps `UseStateForUnknown` so a stale cluster UUID is planned as unchanged when `name` is unchanged. A trailing `planmodifiers.StringSetUnknownIf` marks `id` unknown when `name` in state differs from `name` in config (both known), so an in-place name change can write `<prior-uuid>/<new-name>` without an inconsistent-result error. Role, watch, and datafeed do not need this modifier because their identity keys use RequiresReplace. `name` itself SHALL NOT use RequiresReplace.
