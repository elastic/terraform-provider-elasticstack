...
Force-all prefixes: `provider/`, `internal/acctest/`, `internal/clients/`, `internal/entitycore/`, `generated/`, `xpprovider/`, `.github/workflows/`, `examples/`, `scripts/targeted-testacc/`, `internal/kibana/dashboard/dashboardacctest/`, `internal/kibana/dashboard/panelkit/contracttest/`, `internal/providerfwtest/`. The three shared acceptance-test helper packages are force-all because they are imported only from test files, so the phase-1 reverse-dependency walk (non-test imports) cannot see them. The `provider/` prefix is force-all, and because the provider package is itself an enumerated acceptance test package, a `provider/` change selects the full set including the provider package's own live-stack connection suites. The `examples/` tree is force-all because it is consumed only from test code and sits outside the scope of both selection phases. The `examples` package declares `(go/redacted):embed resources` and `(go/redacted):embed data-sources` (`examples/examples.go`); `internal/acctest/examples_plan_test.go` imports that package and drives `TestAccExamples_planOnly`, a `resource.Test` PlanOnly suite over every embedded example. `BuildImportGraph` runs `go list` over `./internal/... ./provider/...` only, so `examples/` is never a node in the import graph and phase 1 has no reverse-dependency edge into it; it is also outside the enumeration roots, and the unit-test job's `-skip '^TestAcc'` excludes that suite too. (Unlike `internal/providerfwtest/`, which *is* inside the enumeration roots and is invisible specifically because its importers are test files, `examples/` is outside the graph's pattern scope entirely.) An examples-only PR would otherwise select zero packages on every shard. The tool's own package (`scripts/targeted-testacc/`) is force-all because it sits outside the enumerated roots and under no other prefix, so a PR touching only the tool would otherwise select zero acceptance packages — the suite it gates would never be exercised by its own changes, which mirrors the shared test-helper reasoning: it avoids false confidence in partial analysis of the tool's behavior. Tool changes are low-frequency, so the CI cost of the resulting full-suite runs is bounded. A guard unit test SHALL fail when any in-module package is imported only from test files — scanning the test imports of both `internal/` and `provider/`, because a package may be imported only from another root's test files (e.g. `internal/acctest` is test-imported by `provider/`, and `examples/` is test-imported by `internal/acctest`) — and is neither covered by a force-all prefix nor entity-declaring.
...
- **WHEN** `go.mod`, `go.sum`, `Makefile`, `main.go`, `.terraform-version`, `.env.template`, a `docker-compose*.yml` or `docker-compose*.yaml` file, a file under `.github/workflows/`, a file under `examples/`, a file under `scripts/targeted-testacc/`, or a file under a shared acceptance-test helper package (`internal/kibana/dashboard/dashboardacctest/`, `internal/kibana/dashboard/panelkit/contracttest/`, `internal/providerfwtest/`) is changed
- **THEN** the tool emits all acceptance test packages

#### Scenario: Examples-only change triggers full suite

- **WHEN** the only changed files are under `examples/`
- **THEN** the tool emits all acceptance test packages
- **AND** no phase 1 or phase 2 analysis is performed
...
When no changed file matches the force-all prefix or force-all file table and no changed file resolves to a Go package — every changed file is neither a `*.go` file nor inside a Go package tree, i.e. its nearest ancestor directory containing a `.go` file does not exist below the module root (for example, files under `docs/` or `openspec/`) — the tool SHALL emit nothing (zero packages selected) and exit 0. Any changed file inside a Go package tree SHALL be attributed to its nearest ancestor Go package regardless of file type: `go:embed`'ed schema description files (e.g. `internal/elasticsearch/index/index/descriptions/resource.md`) and fixture directories that are not named exactly `testdata` (e.g. `internal/kibana/defaultdataview/test_data/*.tf`) belong to the package that embeds or reads them, so a diff touching only such files selects that package and is not docs-only. A diff that matches the force-all table SHALL always emit the full package set, even when it contains no Go source or testdata content.
...
A guard test SHALL independently enumerate every package under the tool's enumeration roots (`internal/` and `provider/`) that declares a `func TestAcc*` function or invokes `resource.Test`/`resource.ParallelTest` in a `*_test.go` file, using detection that does not share the enumeration's implementation, and SHALL fail when any such package is absent from the tool's acceptance test package enumeration. The guard walk SHALL be derived from the tool's root list, so the guard's scope and the tool's scope stay coupled by construction: content outside the roots (`vendor/`, `.git/`, `scripts/`, docs) is excluded by the walk itself rather than by a manual skip list. The enumeration skips `testdata/` fixture trees inside the roots as a property of the enumeration itself — the go toolchain excludes `testdata/` from package resolution, so a `*_test.go` under a `testdata/` tree is not a resolvable package path for `go list`/`gotestsum` (the trees contain analyzer/test fixtures that textually use `resource.Test` but are not real acceptance suites) — and the guard walk applies the same skip so its scope matches the enumeration's. Because the Run-all threshold, Unresolvable-diff fallback, and Force-all paths all emit exactly the enumerated acceptance test packages, a package missing from the enumeration is unreachable through every selection path; this guard catches acceptance suites that do not follow the `TestAcc` naming convention before they silently stop running. The guard test MAY invoke `go list` and `go env` (it is exempt from the no-git / no-go-list constraint on the pure-logic unit tests).
