...
Force-all prefixes: `provider/`, `internal/acctest/`, `internal/clients/`, `internal/entitycore/`, `generated/`, `xpprovider/`, `.github/workflows/`, `examples/`, `scripts/targeted-testacc/`, `internal/kibana/dashboard/dashboardacctest/`, `internal/kibana/dashboard/panelkit/contracttest/`, `internal/providerfwtest/`. The three shared acceptance-test helper packages are force-all because they are imported only from test files, so the phase-1 reverse-dependency walk (non-test imports) cannot see them. The `provider/` prefix is force-all, and because the provider package is itself an enumerated acceptance test package, a `provider/` change selects the full set including the provider package's own live-stack connection suites. The `examples/` tree is force-all for the same reason as `internal/providerfwtest/` and `scripts/targeted-testacc/`: it is consumed only from test code — `internal/acctest/examples_plan_test.go` embeds `examples/resources/` and `examples/data-sources/` and drives `TestAccExamples_planOnly`, a `resource.Test` PlanOnly suite over every embedded example — so `examples/` sits outside the enumeration roots and outside the `go list` pattern that builds the import graph, so phase 1 has no reverse-dependency edges into it — an examples-only PR would otherwise select zero packages on every shard, and the unit-test job's `-skip '^TestAcc'` excludes that suite too. The tool's own package (`scripts/targeted-testacc/`) is force-all because it sits outside the enumerated roots and under no other prefix, so a PR touching only the tool would otherwise select zero acceptance packages — the suite it gates would never be exercised by its own changes, which mirrors the shared test-helper reasoning: it avoids false confidence in partial analysis of the tool's behavior. Tool changes are low-frequency, so the CI cost of the resulting full-suite runs is bounded. A guard unit test SHALL fail when a package under `internal/` is imported only from test files — scanning the test imports of both `internal/` and `provider/`, because a package under `internal/` may be imported only from `provider/`'s test files — and is neither covered by a force-all prefix nor entity-declaring.
