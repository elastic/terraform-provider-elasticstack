...
The `examples/` tree is force-all because it is consumed only from test code and sits outside the scope of both selection phases. The `examples` package declares `(go/redacted):embed resources` and `(go/redacted):embed data-sources` (`examples/examples.go`); `internal/acctest/examples_plan_test.go` imports that package and runs `TestAccExamples_planOnly`, a plan-only suite over every embedded example. `BuildImportGraph` runs `go list` over `./internal/... ./provider/...` only, so `examples/` is never a node in the import graph and phase 1 has no reverse-dependency edge into it; it is also outside the enumeration roots, and the unit-test job's `-skip '^TestAcc'` excludes that suite too. (Unlike `internal/providerfwtest/`, which *is* inside the enumeration roots and is invisible specifically because its importers are test files, `examples/` is outside the graph's pattern scope entirely.) An examples-only diff would otherwise select zero packages on every shard.
...
**[Risk] Entity name extraction fails on non-literal constructor call sites.**
→ entitycore exports multiple entity-declaring constructors and the tool covers the listed set (see Context). Extraction parses candidate packages with `go/parser` and walks the AST, so formatting drift (multi-line call sites, comments) cannot break it. Call sites whose entity name is not a plain string literal (parameter forwarding, concatenation) are recorded as unresolved sites and guarded by a repo-wide test that fails when one appears outside `internal/entitycore`'s own envelope-internal delegation; constructor drift is caught by the guard test that asserts every exported `entitycore` constructor is classified, and by the guard test for test-only-imported packages. The SDKv2 `elasticstack_elasticsearch_ingest_processor_*` data sources are a known accepted gap (see Context). New envelope constructor shapes need a classification entry in the extraction table — this is a known, low-frequency maintenance burden.

**[Note] Makefile shellcheck suppression scope.** The Makefile carries `# shellcheck disable=SC1073,SC1065,SC1064,SC1072` directives placed directly above each `define`/`$(eval $(call ...))` block that uses make function syntax shellcheck cannot parse; they are scoped to those blocks only (not file-wide). Note that no shellcheck runner is configured or invoked anywhere in this repo — the directives are inert today and exist so a future shellcheck invocation (or a contributor running `shellcheck Makefile` locally) skips these blocks; the gh-aw `.lock.yml` files follow the same write-directives-without-a-runner convention.
