...
The `examples/` tree is force-all for the same reason: it is consumed only from test code. The `examples` package declares `(go/redacted):embed resources` and `(go/redacted):embed data-sources` (`examples/examples.go`); `internal/acctest/examples_plan_test.go` imports that package and runs `TestAccExamples_planOnly`, a plan-only suite over every embedded example. Because `BuildImportGraph` builds its reverse graph from **non-test** `.Imports` only, and `examples/`'s sole importer is a *test* file, phase 1 has no reverse-dependency edge into it — an instance of the same test-only-import blind spot as `internal/providerfwtest/`. An examples-only diff would otherwise select zero packages on every shard, and the unit-test job's `-skip '^TestAcc'` excludes that suite too.
