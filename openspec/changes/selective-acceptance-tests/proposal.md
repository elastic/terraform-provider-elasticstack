...
- `.github/workflows/provider.yml`: `compute-packages` step added; expensive steps gain `if:` conditions; test step switches between `targeted-testacc` and `testacc` based on event type and package availability. Static `shard: [0, 1]` matrix is preserved.
