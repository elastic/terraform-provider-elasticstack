...
- [x] 1.5 Implement default `ImportState` on the envelope as `resource.ImportStatePassthroughID(ctx, path.Root("id"), req, resp)`.
...
- [x] 1.7 Update `internal/entitycore/doc.go` to document the resource envelope alongside the data source envelope (mention the model constraint, callback signatures, and the default ImportState).
...
- [x] 2.6 Remove the explicit `ImportState` method (envelope provides the same passthrough).
...
- [x] 3.6 Run `go test ./internal/elasticsearch/security/systemuser/...`.
...
- [x] 4.6 Preserve the `UpgradeState` method on the concrete resource (envelope does not provide it).
- [x] 4.7 Run `go test ./internal/elasticsearch/security/role/...` plus the role acceptance test.
...
- [x] 5.6 Run `go test ./internal/elasticsearch/security/rolemapping/...`.
