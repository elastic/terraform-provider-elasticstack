...
- [x] 1.1 Add `priorInput jsontypes.Normalized` parameter to `fromAPIModel` in
  `internal/elasticsearch/watcher/watch/models.go`
- [x] 1.2 In the `input` unmarshalling block (lines 138–147), when `priorInput` is known, unmarshal
  ...
  (lines 163–179)
- [x] 1.3 When `priorInput` is unknown or null, keep the existing behavior of marshalling the raw
  API response directly into `d.Input`

## 2. Update `readWatch` call site

- [x] 2.1 In `internal/elasticsearch/watcher/watch/read.go`, update the `fromAPIModel` call to pass
  `state.Input` as the new `priorInput` argument alongside the existing `state.Actions` argument

## 3. Unit test coverage

- [x] 3.1 Add unit tests for the `input` redaction-preservation path; cover:
...
- [x] 4.1 Add an acceptance test in `internal/elasticsearch/watcher/watch/acc_test.go` that:
...
- [x] 4.2 Add any required HCL testdata fixtures under
  `internal/elasticsearch/watcher/watch/testdata/`

## 5. Spec update

- [x] 5.1 Update the delta spec in `openspec/changes/fix-watch-input-redacted-secret/specs/elasticsearch-watch/spec.md` to:
...
- [x] 5.2 After implementation is verified, sync the delta into `openspec/specs/elasticsearch-watch/spec.md` (or archive the change) per project workflow

## 6. OpenSpec validation

- [x] 6.1 Run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate fix-watch-input-redacted-secret --type change` and resolve any reported problems
- [x] 6.2 Confirm `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec status --change "fix-watch-input-redacted-secret" --json` shows no `blocked` state
