...
The repository already prefers checked-in Terraform fixtures for ordinary acceptance steps, and compatibility steps can follow the same fixture discipline without losing the ability to pass `Config` directly to `resource.TestStep`. Tightening this exception now lets the analyzer catch the mechanically detectable class of static inline config that should instead live in checked-in `.tf` files under `testdata/` and be loaded through package-level `go:embed`.
...
- Require compatibility-step `Config` values to resolve to package-level string variables populated by `//go:embed` from a checked-in `.tf` fixture under `testdata/`.
...
- `acceptance-test-config-directory-lint`: compatibility steps that use `ExternalProviders` and `Config` must source that config from package-level embedded `.tf` fixtures under `testdata/` instead of static Terraform strings defined in Go
...
- **Acceptance tests**: SDK/backwards-compatibility tests that currently build static Terraform config in Go will need fixture extraction to `.tf` files under `testdata/` plus package-level `//go:embed` variables.
