...
- [x] 1.2 Resolve identifier-based `Config` expressions to their declarations and accept only package-level `string` variables populated by `//go:embed` from `testdata/.../main.tf`.
...
- [x] 2.2 Extract each static compatibility fixture to `testdata/.../main.tf` and load it through package-level `//go:embed` variables.
