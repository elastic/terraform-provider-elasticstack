...
| **`function_description` enum enforcement** | Not enforced at plan time; API accepts `"min"`, `"max"`, `"mean"` for `metric` detectors but ignores the field for other functions. Use `stringvalidator.OneOf("min", "max", "mean")` to constrain to known values and guard against typos, given these are the only documented values. |
