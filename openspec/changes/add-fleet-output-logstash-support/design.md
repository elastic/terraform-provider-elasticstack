...
- Ensure Logstash behavior supports both SSL-enabled and SSL-disabled configurations, including transitions between modes.
...
- Treat `ssl` as optional for `logstash` and preserve null semantics through create/read/update transitions.
  - Rationale: Fleet supports both secured and unsecured Logstash output configurations; provider behavior must represent both accurately.
  - Alternative considered: requiring SSL for Logstash by convention; rejected because it does not match Fleet capability and restricts valid configurations.
