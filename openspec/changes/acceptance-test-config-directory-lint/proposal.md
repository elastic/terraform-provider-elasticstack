...
- **Acceptance tests**: in-scope tests anywhere in the repo that currently use inline `Config` or raw `config.TestNameDirectory()` will need fixture-oriented updates
- **Regression coverage**: analyzer tests for valid directory-backed steps, valid external-provider exception steps, and invalid mixed or bypassed configurations
