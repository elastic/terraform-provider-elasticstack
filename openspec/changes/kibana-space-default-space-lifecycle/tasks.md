...
- [ ] 1.1 Validate the change with `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate kibana-space-default-space-lifecycle --type change`.
- [ ] 1.2 Sync or archive the delta into `openspec/specs/kibana-space/spec.md` after implementation is verified.

## 2. Destroy Guard for Default Space

- [ ] 2.1 In `internal/kibana/spaces/delete.go`, add a guard at the top of `deleteSpace` that returns early when `resourceID == "default"`, emitting `tflog.Warn(ctx, "default Kibana space cannot be deleted; removing from Terraform state only")`.
- [ ] 2.2 Add `"github.com/hashicorp/terraform-plugin-log/tflog"` to imports in `internal/kibana/spaces/delete.go`.
- [ ] 2.3 Verify that `DELETE /api/spaces/space/default` is not called when `space_id = "default"` and `terraform destroy` runs.
...
- [ ] 4.2 Create the test fixture at `internal/kibana/spaces/testdata/TestAccResourceSpace_DefaultSpace/default_space/main.tf` with:
  ```hcl
...
- [ ] 5.1 Run `make build` to confirm the provider compiles cleanly after changes.
- [ ] 5.2 Run targeted Go unit tests for changed packages: `go test ./internal/kibana/spaces/... ./internal/clients/kibanaoapi/...`.
