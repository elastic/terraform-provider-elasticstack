...
- **Model by-reference using `by_reference.ref_id`, `references_json`, and required `time_range`**, matching `KbnDashboardPanelTypeLensDashboardAppConfig1`.
...
- **Update REQ-025** to clarify that panel-level `config_json` write support does not extend to `lens-dashboard-app`; this panel type is managed through `lens_dashboard_app_config`.
...
- Reference an existing saved Lens visualization using the API-aligned `ref_id` plus `references_json` shape, for example with a reference item whose `name` matches `ref_id`, `type` is `lens`, and `id` is the saved object ID.
...
- **REQ-025 update**: explicitly documents that panel-level `config_json` is not supported for `lens-dashboard-app`.
