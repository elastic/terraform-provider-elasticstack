...
- [x] 2.3 Convert any additional audited compatible Plugin Framework resources outside Fleet and Kibana, or explicitly leave audited outliers for follow-up if they still differ from canonical `resourcecore` behavior. *(Includes audited Elasticsearch PF resources in `resource-inventory.md` except `elasticstack_elasticsearch_security_api_key`.)*
