...
| `out_of_scope` | `elasticstack_elasticsearch_security_api_key` | `elasticsearch` | `security_api_key` | `internal/elasticsearch/security/api_key` | No import | Previously treated as out of scope because of `configuredResources`; that package-level slice was unused and removed, so the resource now uses `resourcecore` |
