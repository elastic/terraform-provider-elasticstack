...
| `dashboards.json` removal | Delete the schema-merging overlay entirely; do not replace it with a full schema overlay. There is no newer Kibana spec version that still includes `lens-dashboard-app`. | Keeping the file and marking `lens-dashboard-app` as removed within it was rejected as fragile and not aligned with the upstream intent. |
| `dashboard-paths.json` overlay | Add a paths-only overlay (`generated/kbapi/dashboard-paths.json`) merged via `injectDashboardAPIPaths` so dashboard CRUD routes remain in the filtered spec. Upstream `oas.yaml` only has redirect stubs for `/api/dashboards/*`. Schemas come from upstream; this file must not reintroduce panel-type schemas. | Re-merging the full `dashboards.json` overlay was rejected: it would keep the mistaken `lens-dashboard-app` types and duplicate upstream schemas. |
| `fixVisualizationIdParam` | Retain in the transformer pipeline. Upstream `/api/visualizations/{id}` redirect stubs still lack `{id}` parameters; without injection, `oapi-codegen` fails. | Removing it was the initial plan when the path lived only in `dashboards.json`; post-removal investigation showed upstream stubs still need the fix. |
...
| Test cleanup | Remove acceptance tests and unit tests that specifically cover `lens-dashboard-app` panels (in `internal/kibana/dashboard/panel/lensdashboardapp/acc_panels_test.go`, `internal/kibana/dashboard/panel/lensdashboardapp/acc_drilldowns_test.go`, and `lens_by_value_embed_wiring_test.go`). No replacement test is required for removed functionality. | Leaving tests in place was rejected: tests for a removed panel type will fail or require ongoing maintenance. |
