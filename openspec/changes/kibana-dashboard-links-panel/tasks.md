...
- [ ] 1.2 Define `SchemaAttribute()` returning `panelkit.PanelConfigBlock(...)` with `panelType = "links"` and `Required: true` (REQ-LINKS-001).
...
- [ ] 1.5 Implement `linksConfigModeValidator` (by_value / by_reference mutual exclusion) and attach to `links_config` block as an `ExtraValidators` entry (REQ-LINKS-001).
- [ ] 1.6 Implement `linksItemTypeValidator` on each link item object enforcing type-specific field isolation (REQ-LINKS-001):
...
- [ ] 3.1 Implement `Handler.ToAPI` (REQ-LINKS-001):
...
- [ ] 3.2 Implement `Handler.FromAPI` using `panelkit.SimpleFromAPI` and a `populateLinksPanelFromAPI` helper (REQ-LINKS-001):
...
- [ ] 4.1 Add `links.Handler{}` to the `panelHandlers` slice (REQ-LINKS-001).
...
- [ ] 5.5 Acceptance test `TestAccKibanaDashboard_LinksPanel_ByValue` (REQ-LINKS-001):
...
- [ ] 5.6 Acceptance test `TestAccKibanaDashboard_LinksPanel_ByReference` (REQ-LINKS-001):
...
- [ ] 6.1 Add the `links_config` schema entry to `openspec/specs/kibana-dashboard/spec.md` (a new numbered REQ referencing REQ-LINKS-001 from this delta spec).
