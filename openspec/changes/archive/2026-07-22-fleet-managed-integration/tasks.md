...
- [x] 1.1 Set the `MinVersion` floor to `9.5.0-SNAPSHOT` in `models.go`/`capabilities.go` (same core as `policyshape.MinVersionCondition`; verified against a 9.5.0-SNAPSHOT Kibana build) — no further spike needed
...
- [x] 4.1 Verify the 9.5.0 `MinVersion` floor established in task 1.1 still applies after the package rename to `managedintegration` (task 3.1); do not re-raise the floor here
- [x] 4.2 Remove the separate `condition`-support capability check (dedicated features resolver and its `SupportsCondition` field) — now redundant with the resource-level 9.5.0 floor, since `policyshape.MinVersionCondition` is also 9.5.0. Delete `capabilities.go`'s dedicated gating; `condition` is unconditionally supported once the resource exists at all.
- [x] 4.3 Update any comments referencing "agentless_policies" to reference "managed_integrations"
...
- [x] 10.3 Run `make gen` to produce committed provider docs including `docs/resources/fleet_managed_integration.md` (experimental resource page is maintained manually alongside default `make docs-generate`; tfplugindocs omits experimental-only resources from the standard pass); `docs/resources/fleet_agentless_policy.md` was never present on this branch (nothing to delete)
- [x] 10.4 Verify the generated docs include the experimental notice and the ECH/Serverless-only note

## 11. Acceptance tests

- [x] 11.1 Rename `TestAccResourceAgentlessPolicy*` functions and acc-test package comments for managed integration; verify composite-ID import strings and destroy checks still target `elasticstack_fleet_managed_integration` (resource type and constants updated in task 3 follow-up)
- [x] 11.2 Rename `testdata/TestAccResourceAgentlessPolicy*` directories to `TestAccResourceManagedIntegration*`; verify fixture layout matches renamed test functions (fixture HCL already uses `elasticstack_fleet_managed_integration`)
- [x] 11.3 Add live-stack acceptance test: in-place `package.version` bump persists via managed_integrations PUT (unit coverage in task 7 / `update_test.go`; this step proves apply against Kibana >= 9.5.0)
- [x] 11.4 Add live-stack acceptance test: in-place `name` change persists end-to-end (plan-only check exists in `TestAccResourceAgentlessPolicy_NameUpdateInPlace`; assert post-apply state/API after task 7 PUT body)
- [x] 11.5 Add live-stack acceptance coverage for `global_data_tags` `number_value` (map + `string_value` shape validated in moved suite / task 5 review fixtures)
- [x] 11.6 Skip-gate acceptance tests against the new version floor established in task 1.1
- [x] 11.7 Add live-stack acceptance test: update with `cloud_connector` in state — assert association survives (PUT re-sends `{enabled, cloud_connector_id}` from prior state per task 7.2; unit test in `update_test.go`)
- [x] 11.8 Add an acceptance test that sets input/stream `condition`, then updates `condition` in place and asserts both values round-trip through create/read/update against a live stack (Kibana >= 9.5.0)
