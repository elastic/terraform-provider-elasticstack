...
Move the `EnforceMinVersion` floor from 9.3.0 to **`9.5.0-SNAPSHOT`** in `models.go` (verified against a 9.5.0-SNAPSHOT Kibana build; same core as `policyshape.MinVersionCondition`). Practitioner-facing diagnostics still name **9.5.0**. CI snapshot stacks satisfy the floor via ordinary semver `GreaterThanOrEqual` against `9.5.0-SNAPSHOT`; no provider-wide Kibana `-SNAPSHOT` uplift in scoped clients.

Because the new floor shares the 9.5.0 core with `MinVersionCondition`, the separate per-request `condition`-support capability check (dedicated features resolver with `SupportsCondition`, and `validateInputConditionSupport` in `models_convert.go`) is now redundant: a stack that can run this resource at all is guaranteed to support `condition`. That capability check, and its dedicated gating, is removed; `condition` is treated as unconditionally supported once the resource-level floor is satisfied.
