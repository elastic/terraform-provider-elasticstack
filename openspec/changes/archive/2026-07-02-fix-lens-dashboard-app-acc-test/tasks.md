...
- [x] 1.1 Remove the `TestAccResourceDashboardUnknownPanel_lensDashboardApp` function and the
       `replaceDashboardPanelWithLensDashboardApp` helper from
       `internal/kibana/dashboard/acc_unknown_panels_test.go`.
- [x] 1.2 Remove any imports that become unused after the deletion (e.g. `bytes`, `io`, `net/http`,
       `context`, `encoding/json`, `fmt` if no longer referenced by other tests in the same file).
- [x] 1.3 If `acc_unknown_panels_test.go` becomes empty after removal, delete the file entirely.

## 2. Verify existing unit coverage

- [x] 2.1 Confirm that `Test_unknownPanelRoundTrip`, the `"unknown panel type preserves id, grid,
...
- [x] 3.1 Keep the delta spec under `openspec/changes/fix-lens-dashboard-app-acc-test/specs/`
