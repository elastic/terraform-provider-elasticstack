...
| Acceptance test gating | Ungated (no version skip, no `solution` attribute). | Gating behind `>= 8.16.0` for `solution` was discussed and rejected by the maintainer: the fixture uses only attributes supported on all stack versions (`space_id`, `name`, `description`), with `description` matching the live default space's value to avoid drift. |
