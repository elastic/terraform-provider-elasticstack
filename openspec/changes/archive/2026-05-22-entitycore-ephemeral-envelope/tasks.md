...
- [x] 1.1 Keep the delta spec at `openspec/changes/entitycore-ephemeral-envelope/specs/entitycore-ephemeral-envelope/spec.md` aligned with `proposal.md` / `design.md`; run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate entitycore-ephemeral-envelope --type change` (or `make check-openspec`).
- [x] 1.2 Confirm whether `providerschema` already exposes an ephemeral-namespaced Kibana connection block helper; if not, plan an addition in implementation task 2.2 below.
- [x] 1.3 On completion of implementation, **sync** delta into `openspec/specs/entitycore-ephemeral-envelope/spec.md` or **archive** the change per project workflow.

## 2. Shared close-state and connection-snapshot infrastructure

- [x] 2.1 Add `internal/entitycore/ephemeral_close_state.go` with:
...
- [x] 2.2 Add `internal/entitycore/ephemeral_connection_snapshot.go` containing:
...
- [x] 2.3 Add `internal/entitycore/ephemeral_close_state_test.go`:
...
- [x] 2.4 Add `internal/entitycore/ephemeral_connection_snapshot_test.go` covering Elasticsearch and Kibana variants:
...
- [x] 3.1 Add `internal/entitycore/elasticsearch_ephemeral_envelope.go` with:
  ...
  - Compile-time interface assertions for `ephemeral.EphemeralResource`, `ephemeral.EphemeralResourceWithConfigure`, and `ephemeral.EphemeralResourceWithClose`.
- [x] 3.2 Add `internal/entitycore/elasticsearch_ephemeral_envelope_test.go` covering:
...
- [x] 4.2 Add `internal/entitycore/kibana_ephemeral_envelope_test.go` mirroring section 3.2 for the Kibana flavor.

## 5. Documentation

- [x] 5.1 Update `internal/entitycore/doc.go` to:
...
- [x] 6.1 Rewrite `internal/elasticsearch/security/api_key/ephemeral_resource.go` on top of `entitycore.NewElasticsearchEphemeralResource[ephemeralTfModel, ephemeralCloseState]`. Steps:
...
- [x] 6.2 Delete `internal/elasticsearch/security/api_key/ephemeral_connection.go` (functionality moved into entitycore).
- [x] 6.3 Delete the now-unused helpers from `ephemeral_resource.go`: `ephemeralPrivateData`, `ephemeralPrivateState`, `saveEphemeralPrivateData`, `loadEphemeralPrivateData`, `elasticsearchConnectionFromPrivateJSON`, the `deleteAPIKeyFn` package-level var and `elasticsearchClientResolver` interface (replaced by entitycore's client resolution), and the `effectiveAPIKeyType` / `invalidateOnCloseValue` helpers (folded into the new Open callback).
- [x] 6.4 Update `internal/elasticsearch/security/api_key/ephemeral_resource_test.go`:
...
- [x] 6.5 Confirm the existing acceptance tests pass unchanged:
...
- [x] 7.1 `make lint`.
- [x] 7.2 `make build`.
- [x] 7.3 `go test ./internal/entitycore/...` (unit tests for the new envelopes).
- [x] 7.4 `go test ./internal/elasticsearch/security/api_key/...` (unit tests for the migrated resource).
- [x] 7.5 Targeted acceptance tests against a live stack (per `dev-docs/high-level/testing.md`); if no stack is reachable locally, rely on PR CI.
- [x] 7.6 `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate entitycore-ephemeral-envelope --type change`.
