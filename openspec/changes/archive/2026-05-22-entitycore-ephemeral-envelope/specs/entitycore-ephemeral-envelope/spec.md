# `entitycore-ephemeral-envelope` — Schema and Functional Requirements

Implementation (new):

- `internal/entitycore/elasticsearch_ephemeral_envelope.go`
- `internal/entitycore/kibana_ephemeral_envelope.go`
- `internal/entitycore/ephemeral_close_state.go`
- `internal/entitycore/ephemeral_connection_snapshot.go`

## Purpose

Provide generic Plugin Framework ephemeral resource envelopes for Elasticsearch- and Kibana-backed entities. The envelopes centralize Configure wiring, Metadata composition, connection-block injection, scoped client resolution, version-requirement enforcement, and the Open/Close lifecycle preludes — including safe round-trip of the optional connection block between `Open()` and `Close()` via envelope-owned private state. Concrete ephemeral resources supply only a schema factory, a typed close-state, an Open callback, and a Close callback; they never touch `Private`.
...
// Kibana flavor: same shape with *clients.KibanaScopedClient and
// KibanaEphemeralModel (GetKibanaConnection() types.List).
```
