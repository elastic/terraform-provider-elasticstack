...
- [x] 1.1 In `internal/fleet/elastic_defend_integration_policy/schema.go`:
...
- [x] 2.1 In `internal/fleet/elastic_defend_integration_policy/models.go`, add
...
- [x] 3.1 In `internal/fleet/elastic_defend_integration_policy/resource.go` (or a new
...
- [x] 3.2 In `internal/fleet/elastic_defend_integration_policy/create.go` (`Create`), before using `agent_policy_ids` from model, check if
...
- [x] 3.3 Apply the same version gate in `internal/fleet/elastic_defend_integration_policy/update.go` (`Update`).

## 4. Request — populate `PolicyIds` in bootstrap and finalize requests

- [x] 4.1 In `internal/fleet/elastic_defend_integration_policy/request.go`, update
...
- [x] 4.2 Apply the same logic to `buildFinalizeRequest`.
- [x] 4.3 Update the signatures of `buildBootstrapRequest` and `buildFinalizeRequest` to accept
  `ctx context.Context` if needed for `ElementsAs`.

## 5. Mapping — populate model from API response

- [x] 5.1 In `internal/fleet/elastic_defend_integration_policy/mapping.go`, update
...
- [x] 6.1 In `openspec/specs/fleet-elastic-defend-integration-policy/spec.md`:
...
- [x] 7.1 Add `TestAccResourceElasticDefendIntegrationPolicy_multiAgentPolicy` in
...
- [x] 7.2 Add a version-gate test (or skip annotation) that asserts that `agent_policy_ids`
  returns an appropriate error on stacks older than 8.15.0.
- [x] 7.3 Verify that existing tests using `agent_policy_id` continue to pass unmodified.

## 8. Build and validation

- [x] 8.1 `make build` — provider compiles without errors.
- [x] 8.2 `go test ./internal/fleet/elastic_defend_integration_policy/... -v` — package tests pass.
- [x] 8.3 `make check-openspec` — spec validation passes.
