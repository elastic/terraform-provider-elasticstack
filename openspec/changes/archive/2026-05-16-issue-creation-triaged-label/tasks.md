...
- [x] 1.1 In `.github/workflows-src/schema-coverage-rotation/workflow.md.tmpl` (line 43), change:
  ```
...
- [x] 2.1 In `.github/workflows-src/duplicate-code-detector/workflow.md.tmpl` (line 31), change:
  ```
...
- [x] 3.1 In `.github/workflows-src/semantic-function-refactor/workflow.md.tmpl` (line 29), change:
  ```
...
- [x] 4.1 In `.github/workflows-src/flaky-test-catcher/workflow.md.tmpl` (line 31), change:
  ```
  labels: [flaky-test]
  ```
  to:
...
- [x] 5.1 Run `make workflow-generate` to regenerate all four `.lock.yml` files from the updated
  source templates:
...
- [x] 5.2 Verify that only the expected label additions appear in the compiled diff (the
...
- [x] 6.1 Run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate issue-creation-triaged-label --type change` and resolve any reported issues.
- [x] 6.2 Verify that the `triaged` GitHub label exists in the repository before the PR lands (the
  Issue Classifier already applies it, so it should exist).
- [x] 6.3 Confirm `make build` still passes if any non-workflow source files were modified
