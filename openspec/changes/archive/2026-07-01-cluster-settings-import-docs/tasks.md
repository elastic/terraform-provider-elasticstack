...
- [x] 1.1 Create `examples/resources/elasticstack_elasticsearch_cluster_settings/import.sh` with:
...
- [x] 2.1 Run `make docs-generate` to regenerate `docs/resources/elasticsearch_cluster_settings.md`
- [x] 2.2 Verify that the regenerated file contains a `## Import` section with the correct import command

## 3. Sync spec

- [x] 3.1 Run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec sync cluster-settings-import-docs` to merge REQ-020 from the delta spec into `openspec/specs/elasticsearch-cluster-settings/spec.md`
- [x] 3.2 Verify the main spec now includes REQ-020 and that `openspec validate elasticsearch-cluster-settings --type spec` passes
