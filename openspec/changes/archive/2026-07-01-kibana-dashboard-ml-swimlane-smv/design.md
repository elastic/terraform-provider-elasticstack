...
1. ~~**Minimum Kibana version for `ml_anomaly_swimlane` and `ml_single_metric_viewer` panel types**~~ — **Resolved**: the underlying ML embeddables predate the Dashboard API (swim lane: Kibana 7.9.0; single metric viewer: Kibana 8.13.0), but no minimum version specific to their typed representation in the Dashboard API's panel schema could be confirmed from release notes. Both types are already present in the generated `kbapi` client this resource depends on, and no other typed panel block in this resource carries a bespoke per-panel version gate (version gates in this codebase are reserved for fields added incrementally to an already-shipped API surface, e.g. alerting rule `alert_delay`/`flapping`, not for panel types already exposed by the generated client). Decision: no additional version gate is added; the Kibana Dashboard API rejects the panel on incompatible stack versions. **Empirically confirmed during implementation** (task 4): Kibana 9.4.0 rejects both panel types with a 400 (type not in the Dashboard API's enum); Kibana 9.5.0-SNAPSHOT (the `kibana/main` branch build the generated `kbapi` client tracks) accepts both and round-trips correctly via acceptance tests. Documented as a compatibility note in the delta spec (REQ-047, REQ-048).
