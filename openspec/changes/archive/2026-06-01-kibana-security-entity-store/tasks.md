...
- [x] 1.1 Register `elasticstack_kibana_security_entity_store` in the provider's resource list
  in `provider/plugin_framework.go`.
- [x] 1.2 Register `elasticstack_kibana_security_entity_store_status` in the provider's data
  source list in `provider/plugin_framework.go`.

## 2. Package scaffold

- [x] 2.1 Create directory `internal/kibana/security_entity_store/`.
- [x] 2.2 Create the following empty files (stubs) so the package compiles:
...
- [x] 3.1 In `schema.go`, define `getSchema()` returning a `schema.Schema` with:
...
- [x] 4.1 Define `tfModel` struct with `tfsdk` tags matching all schema attributes.
- [x] 4.2 Define nested structs `historySnapshotModel` and `logExtractionModel`.
- [x] 4.3 Implement `KibanaResourceModel` interface methods on `tfModel`:
  - `GetID() types.String`
...
- [x] 5.1 Declare `var MinVersion = version.Must(version.NewVersion("9.1.0"))`.
- [x] 5.2 Construct the resource via `entitycore.NewKibanaResource[tfModel]` with callbacks:
  `Schema: getSchema`, `Create: createEntityStore`, `Read: readEntityStore`,
  `Update: updateEntityStore`, `Delete: deleteEntityStore`.
- [x] 5.3 Assert `resource.Resource`, `resource.ResourceWithConfigure`, and
  `resource.ResourceWithImportState` are satisfied.

## 6. Create callback — `create.go`

- [x] 6.1 Enforce `MinVersion` at 9.1.0 before any API call.
- [x] 6.2 Build `PostSecurityEntityStoreInstallJSONRequestBody` from plan:
...
- [x] 6.3 Call `PostSecurityEntityStoreInstallWithResponse`; accept HTTP 200 and 201 as success;
  return error diagnostic for other codes.
- [x] 6.4 If `started == false`, call `PutSecurityEntityStoreStopWithResponse`.
- [x] 6.5 Compute `id` as `<space_id>/entity_store` and set in state.
- [x] 6.6 Call the Read callback to populate all computed fields.

## 7. Read callback — `read.go`

- [x] 7.1 Call `GetSecurityEntityStoreStatusWithResponse` with `IncludeComponents: false` (or true if
  needed for extraction settings).
- [x] 7.2 If response status is `not_installed`, call `resp.State.RemoveResource(ctx)` and return.
- [x] 7.3 Collect engine `Type` values and populate `entity_types` in state.
- [x] 7.4 Set `started = true` if any engine has status `running`, else `false`.
- [x] 7.5 Populate `log_extraction` from the first engine's extraction settings (delay, frequency,
  lookback_period, field_history_length, max_logs_per_page); leave nil fields as null.
- [x] 7.6 Serialize the full status response body to `status_json` (normalized via `json.Marshal`
  after unmarshalling, or use the raw `Body` bytes stored via `json.RawMessage`).

## 8. Update callback — `update.go`

- [x] 8.1 Enforce `MinVersion` at 9.1.0.
- [x] 8.2 **Log extraction reconciliation**: if `log_extraction` changed between prior state and
...
- [x] 8.3 **Entity type expansion**: compute added types (in plan but not in state). If non-empty,
  call `PostSecurityEntityStoreInstallWithResponse` with the full desired `entity_types`.
- [x] 8.4 **Entity type shrink guard**: compute removed types (in state but not in plan).
...
- [x] 8.5 **Start/stop reconciliation**: if `started` changed from true to false, call
  `PutSecurityEntityStoreStopWithResponse`; from false to true, call
  `PutSecurityEntityStoreStartWithResponse`.
- [x] 8.6 Call the Read callback to refresh state.

## 9. Delete callback — `delete.go`

- [x] 9.1 Call `PostSecurityEntityStoreUninstallWithResponse` with the entity types from state
  (nil body uninstalls all, which is equivalent to passing all installed types).
- [x] 9.2 Accept HTTP 200 as success. Remove resource from state implicitly (framework does this).

## 10. Data source — `data_source.go`, `data_source_schema.go`, `data_source_models.go`, `data_source_read.go`

- [x] 10.1 In `data_source_schema.go`, define `getDataSourceSchema()` with:
...
- [x] 10.2 In `data_source_models.go`, define `dsModel` struct.
- [x] 10.3 In `data_source_read.go`, implement the read callback:
...
- [x] 10.4 In `data_source.go`, register the data source via the provider's data source list.

## 11. Import support

- [x] 11.1 Implement `ImportState` on the resource by delegating to
...
- [x] 12.1 `TestAccResourceKibanaSecurityEntityStore_basic` — create with default entity types,
  verify plan is clean after apply.
- [x] 12.2 `TestAccResourceKibanaSecurityEntityStore_singleType` — create with `entity_types = ["host"]`.
- [x] 12.3 `TestAccResourceKibanaSecurityEntityStore_updateLogExtraction` — update `log_extraction`
  settings and verify no replacement.
- [x] 12.4 `TestAccResourceKibanaSecurityEntityStore_import` — import the resource using its ID.
- [x] 12.5 `TestAccResourceKibanaSecurityEntityStore_shrinkGuardFails` — verify that reducing
  `entity_types` without `allow_entity_type_shrink = true` produces an error.
- [x] 12.6 `TestAccResourceKibanaSecurityEntityStore_shrinkWithFlag` — verify that reducing
  `entity_types` with `allow_entity_type_shrink = true` succeeds.
- [x] 12.7 `TestAccResourceKibanaSecurityEntityStore_startedFalse` — create with `started = false`
  and verify engines are not running.
- [x] 12.8 `TestAccDataSourceKibanaSecurityEntityStoreStatus_basic` — verify the data source reads
  status with and without `include_components`.

## 13. OpenSpec spec update

- [x] 13.1 Apply the delta specs for capabilities `kibana-security-entity-store` and
...
- [x] 14.1 `make build` — provider compiles without errors.
- [x] 14.2 `go test ./internal/kibana/security_entity_store/... -v` — unit tests pass.
- [x] 14.3 `make check-openspec` — spec validation passes.
