...
- `internal/kibana/dashboard/lenscommon/partition_alignment.go` — widen `PartitionValueDisplayMatchesKibanaDefault` for `percent_decimals`; `AlignPartitionLegendStateFromPlan` also preserves treemap/mosaic `legend.truncate_after_lines` / `nested`.
...
- `internal/kibana/dashboard/panel/lensheatmap/` alignment path — align axis label `orientation` and `legend.truncate_after_lines` from plan.
