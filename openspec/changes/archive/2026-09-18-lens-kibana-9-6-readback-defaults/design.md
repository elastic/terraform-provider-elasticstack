...
- Introducing a general "treat all Kibana defaults as unknown-after-apply" mechanism. The issue's "likely fix" section floats this as an alternative, but it would require making Lens `config_json`/typed fields `Computed` with `UseStateForUnknown`-style semantics, a materially larger schema change than the targeted default-normalization this change proposes. See "Resolved questions" below.
- Rewriting the XY reference-line `thresholds[].axis` Terraform schema enum and validator (`internal/kibana/dashboard/panel/lensxy/schema_attrs.go` still validates `bottom|left|right`) to match Kibana 9.6's accepted values (`"y"`, `"y2"`). MarkdownDescription and generated docs already record that Kibana 9.6 accepts `y`/`y2` and rejects those schema values. Live 9.6 rejects `left`/`right`/`bottom` with HTTP 400 — a schema-contract mismatch, not an omit-default. Schema-contract follow-up is tracked as https://github.com/elastic/terraform-provider-elasticstack/issues/4958. The ES|QL XY static-color overwrite is tracked as [elastic/kibana#291451](https://github.com/elastic/kibana/issues/291451); this axis enum mismatch is the other owned discovery from the 9.6 probes.
...
- **`percent_decimals` default is `2`.** Live 9.6 probes confirmed a single default that does not vary by metric number `format` (including `format.type = "percent"`). REQ-011 now treats `{mode="percentage", percent_decimals=2}` as the 9.6 default shape (and still accepts the older `{mode="percentage", percent_decimals=null}`). When the practitioner omitted `percent_decimals` and Kibana returns `2`, the resource preserves the null plan value regardless of `mode`.
- **Heatmap axis-label `orientation` default is `"horizontal"` on X only, not `{x,y}`.** Y has no `orientation` in the schema or API. This is a distinct field from the already-documented `labels.visible` / `title.visible` heatmap defaults (REQ-011) — same mechanism (`PreserveNullIfStateEquals`), new field.
...
Answers from live 9.6 probes.

- **Metric `axis` injection.** The only injected omit-default is `"y"`. Kibana never infers `"y2"` from chart-level `axis.y2`; if the practitioner already set `axis` on the metric JSON (including `"y2"`), Kibana preserves it. There is no X-axis metric path (`x`/`x_json` is a dimension). Implementation treats omitted metric `axis` as default `"y"` and leaves an explicit metric `axis` alone.
- **`percent_decimals`.** Fixed Kibana default `2`, not format-aware. Codified in REQ-011.
- **Legacy `metric_json`.** Not explained by a shared `axis`/`color` gap alone. Kibana 9.6 does not inject `axis` on legacy metric. It injects `color: {"type":"auto"}` (shared default, now applied) and legacy-only `size: "m"`. The live probe used `operation: "count"`; remaining field-metric operations are inferred from the shared Kibana metric schema.

## Risks / Trade-offs

- [Resolved] Hard-coding `axis: "y"` in `PopulateLensMetricDefaults` would have been wrong for secondary-axis or X-axis metrics. Mitigated: axis injection is confined to `PopulateXYMetricDefaults` (XY `y[]` only); `PopulateLensMetricDefaults` still injects no axis (`TestPopulateLensMetricDefaults_doesNotInjectAxis`). Live probes retired the X-axis-metrics half of the concern (`x`/`x_json` is a dimension). `_axis` passes on 9.6.
...
- Upstream: [elastic/kibana#291451](https://github.com/elastic/kibana/issues/291451).
