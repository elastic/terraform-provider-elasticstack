...
- Make apply and the following plan succeed when a logging action omits `level`.
...
| Logging `level` | When an object has a `"logging"` key whose value is an object, copy that object and fill `level: "info"` if `level` is absent. Do not treat arbitrary objects that happen to have `level` as logging actions. |
| Absent vs explicit | Only fill keys that are **absent**. Explicit `rest_total_hits_as_int: false`, `search_type: "dfs_query_then_fetch"`, non-empty `indices`, non-painless `lang`, or non-info logging `level` are left untouched so real drift still compares unequal. |
...
- **Over-broad `logging` matching**: a nested object keyed `logging` that is not a Watcher logging action would get `level` for comparison only. Same acceptance.
