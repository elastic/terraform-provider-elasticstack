...
- [x] 4.5 Confirm the existing role privilege set (`fleet`/`fleetv2` all, scoped to the test space) works against a running 9.1+ stack — adjust only if the restricted user 403s on the legitimately-scoped path too. *(Confirmed by CI: `TestAccReproduceIssue4282` PASS in Matrix Acceptance Test (9.1.10) on head `88a8031c` — the restricted user reads the target space and 403s only on the default space.)*
...
- [x] 5.4 Bring up a 9.1+ Elastic Stack and run `make testacc TESTARGS='-run TestAccReproduceIssue4282'` (or the repo's docker-testacc equivalent); confirm it fails pre-fix (403 during post-install wait) and passes post-fix. *(Post-fix confirmed by CI: `TestAccReproduceIssue4282` PASS (16.04s) in Matrix Acceptance Test (9.1.10) on head `88a8031c`; the pre-fix 403 was captured by the original `ExpectError` reproducer earlier on this branch.)*
- [x] 5.5 Re-run `TestAccResourceIntegration_MultiSpaceInstall`, `TestAccResourceIntegration_MultiSpaceDelete`, and `TestAccResourceIntegration_SpaceAwareDrift` to confirm no regression from the `spaceScope` refactor. *(Confirmed by CI: all three PASS in Matrix Acceptance Test (9.1.10) on head `88a8031c`, alongside 9.4.2 and the 8.x matrix.)*
