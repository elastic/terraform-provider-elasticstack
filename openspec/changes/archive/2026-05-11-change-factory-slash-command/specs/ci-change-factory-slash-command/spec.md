...
Define requirements for the `/change-factory` slash command trigger that allows maintainers to invoke `change-factory` issue intake from an issue comment and pass freeform human direction as an authoritative design override to the proposal agent.
