# `ci-workflow-shared-imports` — shared pre-activation script consolidation

## Purpose

When multiple GitHub Agentic Workflows inline the same script logic via `x-script-include`, the repository SHALL keep a single canonical copy of that script rather than duplicating it across workflow directories. This removes drift risk and ensures a single maintenance point for shared logic.
