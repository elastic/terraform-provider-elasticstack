...
- [x] 1.1 Add `var MinSupportedVersion = version.Must(version.NewVersion("9.1.0"))` to
      `internal/elasticsearch/index/datastreamoptions/version_gating.go`.
- [x] 1.2 Add the `GetVersionRequirements(tmplObj types.Object) ([]entitycore.VersionRequirement, diag.Diagnostics)`
...
- [x] 1.3 Delete the `EnforceMinServerVersion` function from `datastreamoptions/version_gating.go`
...
- [x] 2.1 Add `GetVersionRequirements() ([]entitycore.VersionRequirement, diag.Diagnostics)` to
...
- [x] 2.2 Update imports in `componenttemplate/models.go` to include
...
- [x] 3.1 Delete the `serverVersion, sdkDiags := client.ServerVersion(ctx)` block and the
...
- [x] 3.2 Remove the now-unused `diagutil` and `datastreamoptions` imports from `componenttemplate/create.go`
...
- [x] 4.1 Add `GetVersionRequirements() ([]entitycore.VersionRequirement, diag.Diagnostics)` to
...
- [x] 4.2 Update imports in `template/models.go` to include `entitycore` and `datastreamoptions`.

## 5. Replace `serverVersion` block in `template/create.go`

- [x] 5.1 Delete the following block from `internal/elasticsearch/index/template/create.go`:
      ```go
      ...
      if resp.Diagnostics.HasError() { return }
      ```
- [x] 5.2 Add the following replacement block immediately after the client is obtained and before
      `plan.toAPIModel`:
      ```go
      ...
      ```
- [x] 5.3 Remove the now-unused `datastreamoptions` import from `template/create.go`.

## 6. Replace `serverVersion` block in `template/update.go`

- [x] 6.1 Apply the same replacement (task 5.1 → 5.2) to the equivalent block in
      `internal/elasticsearch/index/template/update.go` (lines 57–67 in the current file).
- [x] 6.2 Remove the now-unused `datastreamoptions` import from `template/update.go`.

## 7. Delete `template/version_gating.go`

- [x] 7.1 Verify that `validateIgnoreMissingComponentTemplatesVersion` has no remaining callers
      (it should have none after tasks 5 and 6).
- [x] 7.2 Delete `internal/elasticsearch/index/template/version_gating.go`.

## 8. Add unit tests — `componenttemplate/version_requirements_test.go`

- [x] 8.1 Create `internal/elasticsearch/index/componenttemplate/version_requirements_test.go` with
...
- [x] 9.1 Create `internal/elasticsearch/index/template/version_requirements_test.go` with
...
- [x] 10.1 Refactor the test at lines 211–227 in
...
- [x] 10.2 Remove the `datastreamoptions` import from `expand_flatten_test.go` if it becomes unused.

## 11. Requirements update

- [x] 11.1 Update REQ-027 in `openspec/specs/elasticsearch-index-component-template/spec.md` to reflect
...
- [x] 11.2 Update REQ-012 in `openspec/specs/elasticsearch-index-template/spec.md` to specify that
      `ignore_missing_component_templates` version gating uses `Model.GetVersionRequirements()` and
      `client.EnforceMinVersion`.
- [x] 11.3 Update REQ-033 in `openspec/specs/elasticsearch-index-template/spec.md` to specify that
...
- [x] 12.1 Run `make build` to confirm the provider compiles without errors.
- [x] 12.2 Run `go test ./internal/elasticsearch/index/componenttemplate/... ./internal/elasticsearch/index/template/...`
      to confirm all unit tests pass.
- [x] 12.3 Run `make check-lint` to confirm lint passes.
