# `ci-factory-pipeline-phase-labels` — Persistent phase labels for factory pipeline stages

Workflow implementation: `.github/workflows-src/lib/set-phase-label.js` (shared helper) and
`set_phase_label.inline.js` scripts in each factory's `scripts/` directory.

## Purpose

Define requirements for the shared `setPhaseLabel` helper and the `set_phase_label` pre-activation
step that each factory workflow adds to persistently track an issue's pipeline stage through a
`phase-*` label.

## Schema

```yaml
# Label set (must be created manually in the GitHub repository)
labels:
