...
- **`internal/entitycore/resource_timeouts.go`** (new file): Introduce shared timeouts plumbing usable by both envelopes:
  - `ResourceTimeoutsField` — embeddable struct with field `Timeouts timeouts.Value` tagged `tfsdk:"timeouts"` (using `github.com/hashicorp/terraform-plugin-framework-timeouts/resource/timeouts`) and a `GetTimeouts()` value-receiver method
  - `WithResourceTimeouts` interface — `GetTimeouts() timeouts.Value`
  - `ResourceTimeouts` struct with fields `Create, Read, Update, Delete time.Duration` for per-op default overrides; any zero field falls back to the corresponding `DefaultResource<Op>Timeout` constant
