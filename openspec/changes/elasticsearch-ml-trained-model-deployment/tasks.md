...
- [ ] 1.2 Resolve open questions in `design.md` (adaptive allocations validation, `wait_for` default, force-stop flag, stats-query filtering by deployment_id); update delta spec with confirmed behaviour.
...
- [ ] 3.3 Implement `schema.go`: define `GetSchema()` returning the Plugin Framework schema. Mark ForceNew attributes with `RequiresReplace`. Add plan modifier on `number_of_allocations` to suppress diff when `adaptive_allocations.enabled = true`.
...
- [ ] 3.5 Implement `create.go`: call `StartTrainedModelDeployment` with user-specified parameters; if `wait_for` is set, poll `GetTrainedModelStats` until allocation status matches or `timeouts.create` elapses (reuse `internal/asyncutils/state_waiter.go` pattern); populate computed attributes (id, deployment_id, state, allocation_status, stats_json).
- [ ] 3.6 Implement `read.go`: call `GetTrainedModelStats`; populate `state`, `allocation_status`, `stats_json`; when `adaptive_allocations.enabled = true`, preserve state's `number_of_allocations` (do not overwrite from API).
...
- [ ] 5.5 Add acceptance test: **Update `adaptive_allocations`** — enable adaptive allocations; assert `number_of_allocations` diff is suppressed on subsequent plan.
...
- [ ] 5.9 Add unit tests for plan modifier: assert `number_of_allocations` diff is suppressed when `adaptive_allocations.enabled = true`, and surfaces when `adaptive_allocations.enabled = false`.
- [ ] 5.10 Add unit tests for read path: assert `number_of_allocations` is preserved from state when adaptive allocations are enabled, and updated from API when disabled.
