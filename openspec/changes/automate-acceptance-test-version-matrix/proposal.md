...
- Rewrite `acceptance-test-isolation` REQ-ACC-002, which today pins the literal string `"9.4.2"` in the matrix, to a version-range invariant ("exactly one 9.4.x GA entry, immediately before the newest snapshot entry") against the new pinned artifact, so the first automated 9.4.x patch bump does not violate the spec.
...
- `acceptance-test-isolation`: REQ-ACC-002 changes from pinning the exact string `"9.4.2"` in `.github/workflows/provider.yml` to a version-range invariant against `.github/versions/acceptance-test-matrix.json` ("exactly one 9.4.x GA entry"), so it survives automated patch bumps.

## Impact

- **New code**: `scripts/version-matrix/` (Go engine: GA-tag discovery, SNAPSHOT-label lookup, optional image-existence probe, pinned-artifact diffing/rewriting).
