...
**Why:** The agentless policy body has nearly identical inputs/streams/vars structure to an integration policy. Duplicating the Plugin Framework custom types and the normalization logic would create maintenance debt immediately. Extracting first (Phase 1) and migrating `integration_policy` to use the shared package is a behaviour-preserving refactor that can be validated independently before Phase 2 begins. The name `policyshape` is a working name; implementers should validate it against existing naming conventions in the repo.
