...
- **THEN** Elasticsearch SHALL be reachable from the agentic sandbox at `http://host.docker.internal:9200` (proxied through `es-proxy` on port `9201`)

#### Scenario: Kibana reachable from agent sandbox
- **WHEN** the workflow starts the elastic-stack shared services
- **THEN** Kibana SHALL be reachable from the agentic sandbox at `http://host.docker.internal:5601` (proxied through `kb-proxy` on port `5602`)
