...
`TestAccResourceSecurityRole` fails on Elasticsearch 9.5.0-SNAPSHOT because the server now returns `"data_source": []` as a top-level entry inside the `global` privilege object — an array where the go-elasticsearch typed client expects `map[string]map[string][]string`. The typed client's `Role.UnmarshalJSON` cannot decode this heterogeneous shape and raises a JSON unmarshal error, breaking every `GetRole` read that returns a role with `global` privileges on ES 9.5+.
...
- `elasticsearch-security-role`: modify REQ-001–REQ-003 / REQ-017 — the `GetRole` implementation SHALL bypass the typed client's `Role.Global` field and fetch the `global` object via raw transport, decoding it as `json.RawMessage`. The existing requirement language for typed API usage is narrowed: PutRole and DeleteRole continue to use the typed API; GetRole uses the typed API for all fields except `global`, which is fetched via raw transport.
...
- `internal/clients/elasticsearch/security_role.go` — replace `Security.GetRole().Do(ctx)` with a raw `/_security/role/<name>` GET via `typedClient.Transport.Perform`; decode response into a partial typed struct, using `json.RawMessage` for the `global` field then marshaling it into a `map[string]map[string]map[string][]string`-compatible alias. Role data returned to callers as `*types.Role` with `Global` populated from the raw JSON.
