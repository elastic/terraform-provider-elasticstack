...
- `color` — Optional+Computed string; a hex color value (e.g. `#772299`). When absent from configuration, the server generates a random color and `UseStateForUnknown` prevents spurious diffs on subsequent plans.
