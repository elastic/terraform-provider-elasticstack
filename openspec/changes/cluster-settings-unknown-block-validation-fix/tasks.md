...
- [ ] 1.1 In `internal/elasticsearch/cluster/settings/resource.go`, update `categoryBlockEmpty` to return `false` when `block.IsUnknown()`, instead of `true`. Remove `block.IsUnknown()` from the combined `IsNull() || IsUnknown()` check and add a separate early-return branch.
