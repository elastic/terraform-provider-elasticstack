...
- **`percent_decimals` default is `2`.** Live 9.6 probes confirmed a single default that does not vary by metric number `format` (including `format.type = "percent"`). REQ-011 now treats `{mode="percentage", percent_decimals=2}` as the 9.6 default shape (and still accepts the older `{mode="percentage", percent_decimals=null}`). When the practitioner omitted `percent_decimals` and Kibana returns `2`, the resource preserves the null plan value regardless of `mode`. See findings.md § 1.2.
...
Answers from live 9.6 probes; quotes in `findings.md` §§ 1.1–1.3.

- **Metric `axis` injection (findings.md § 1.1).** The only injected omit-default is `"y"`. Kibana never infers `"y2"` from chart-level `axis.y2`; if the practitioner already set `axis` on the metric JSON (including `"y2"`), Kibana preserves it. There is no X-axis metric path (`x`/`x_json` is a dimension). Implementation treats omitted metric `axis` as default `"y"` and leaves an explicit metric `axis` alone.
- **`percent_decimals` (findings.md § 1.2).** Fixed Kibana default `2`, not format-aware. Codified in REQ-011.
- **Legacy `metric_json` (findings.md § 1.3).** Not explained by a shared `axis`/`color` gap alone. Kibana 9.6 does not inject `axis` on legacy metric. It injects `color: {"type":"auto"}` (shared default, now applied) and legacy-only `size: "m"`.

## Risks / Trade-offs

- [Resolved] Hard-coding `axis: "y"` in `PopulateLensMetricDefaults` would have been wrong for secondary-axis or X-axis metrics. Mitigated: axis injection is confined to `PopulateXYMetricDefaults` (XY `y[]` only); `PopulateLensMetricDefaults` still injects no axis (`TestPopulateLensMetricDefaults_doesNotInjectAxis`). Findings §1.1 retired the X-axis-metrics half of the concern (`x`/`x_json` is a dimension). `_axis` passes on 9.6.
