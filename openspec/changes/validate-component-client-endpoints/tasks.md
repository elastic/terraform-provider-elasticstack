...
- [ ] 2.1 Update `(*clients.ElasticsearchScopedClient).GetESClient()` to reject missing effective Elasticsearch endpoints with the new actionable configuration error.
- [ ] 2.2 Update `(*clients.KibanaScopedClient).GetKibanaClient()` and `GetKibanaOapiClient()` to reject missing effective Kibana endpoints with component-specific actionable errors and without relying on legacy localhost defaults.
- [ ] 2.3 Update `(*clients.KibanaScopedClient).GetFleetClient()` to reject missing effective Fleet endpoints with the Fleet-specific actionable error while preserving Kibana-derived Fleet endpoint resolution.
