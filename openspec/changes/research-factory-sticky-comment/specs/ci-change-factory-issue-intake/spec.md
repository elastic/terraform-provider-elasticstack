...
### Requirement: Agent uses the implementation-research comment as the exclusive scope when present
When the triggering issue has a comment authored by `github-actions[bot]` that contains the marker `<!-- gha-research-factory -->` and a heading `## Implementation research`, the `change-factory` agent SHALL treat that entire comment as the exclusive authoritative source for proposal scope. The agent SHALL adopt the comment's `### Recommendation` as the spine of the OpenSpec proposal it authors, SHALL carry the comment's `### Open questions` verbatim into the resulting `design.md` (under a section such as `## Open questions`), and SHALL use the comment's `### Approaches considered` for context only — the agent SHALL NOT re-explore alternative approaches the research has already evaluated unless the issue body or its comments contradict the recommendation. When no such comment exists on the issue, the agent SHALL retain the existing behavior of treating the issue title and body as the authoritative source.
