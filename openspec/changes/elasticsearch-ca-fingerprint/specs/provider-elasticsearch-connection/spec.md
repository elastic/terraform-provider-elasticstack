...
When `ca_fingerprint` is set, the provider SHALL set `elasticsearch.Config.CertificateFingerprint` to the supplied value so the client uses the SHA-256 hex fingerprint to pin the server certificate. The attribute description SHALL state that the expected format is a 64-character SHA-256 hexadecimal string without colons or separators.
