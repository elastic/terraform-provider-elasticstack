...
- `searchable_snapshot.force_merge_on_clone` when `false`: Elasticsearch `9.2.0`
...
searchable_snapshot {
  ...
  force_merge_on_clone = <optional + computed, bool> # default true; false requires ES >= 9.2
}
```
...
- AND the connected Elasticsearch server is below `9.2.0`
- WHEN the policy is expanded
- THEN the provider SHALL return an unsupported-setting diagnostic
- AND the provider SHALL NOT call the Put Lifecycle API

#### Scenario: Default searchable-snapshot force-merge-on-clone omitted pre-9.2
...
- GIVEN a `searchable_snapshot` block configures `force_merge_on_clone = false`
- AND the connected Elasticsearch server is `9.2.0` or above
- WHEN the policy is expanded
