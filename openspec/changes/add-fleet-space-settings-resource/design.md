...
| Unit | Responsibility |
|---|---|
| `schema.go` | `space_id` is a required string with a replace-on-change modifier, defined locally because the shared `kbschema` space attributes are optional-with-default; `allowed_namespace_prefixes` is a required list of strings with a maximum size of 10 and unique-values validation; `managed_by` and `id` are computed. |
| `models.go` | Model exposing `space_id` as the resource ID, the write ID and the space for envelope routing. Provides the version requirement (minimum 9.1.0) and mapping between the API item and the model, mapping a missing API list to an empty list. |
| `create.go` | A single write callback, used for both create and update (no separate `update.go`), that sends the planned list in one PUT for the planned space. |
| `read.go` | GET for the space; returns "not found" when the client wrapper returns nil. |
| `delete.go` | PUT with an empty list for the space; a not-found response is treated as success. |
| `internal/clients/fleet/space_settings.go` | Wrapper functions `GetSpaceSettings` / `PutSpaceSettings` / `ResetSpaceSettings` (PUT of an empty list where a 404 counts as success, used by destroy) using the existing typed-response helpers and space-aware path editor; GET returns nil on HTTP 404. |
...
Plan → write callback issues a single PUT (space in the request path) → the envelope performs a read-after-write GET → state is built exclusively from the GET response: the list keeps the API order, a missing list becomes an empty list, `managed_by` is null when absent. Destroy issues a PUT with an empty list and drops the resource from state.
...
- Plan-time validation (more than 10 elements, duplicates) prevents API calls entirely.

### Testing approach

- Unit tests for the model/API mapping (empty, missing list, present and absent `managed_by`), the schema validators (11 elements, duplicates) and the version requirement.
- Acceptance tests in the package against a stack of at least 9.1: create, update, import by space ID, destroy resets the list while the space remains, drift after an out-of-band change, and a non-default space (created with `elasticstack_kibana_space`). The default-space import test writes into the shared `default` space of the test stack and relies on destroy to reset it. Tests follow the repo's testing docs and skip on older stack versions.
...
- [Two Terraform resources target the same space and overwrite each other's list] → Document that exactly one `elasticstack_fleet_space_settings` should exist per space; no provider-side detection.
- [Destroy removes a security control] → Document that destroy resets the list to empty and therefore lifts the namespace restriction.
...
- [Kibana may reorder or normalise prefixes, causing drift with an ordered list] → The list mirrors the API order; if acceptance tests show reordering, revisit the attribute type.
- [Values above the 10-element write limit already stored in Kibana (up to 100 readable) cannot be written back by Terraform] → Read shows them as drift; the plan-time validation blocks applying an oversized list. No workaround is provided (see proposal non-goals).
