...
**Migration**: ImportState remains opt-in under `Envelope keeps ImportState opt-in`. Concrete resources that previously supplied thin Create and Update wrappers SHALL migrate to create and update callbacks passed to `NewElasticsearchResource`, except where the design-approved pattern applies: the resource embeds `ElasticsearchResource`, passes non-nil placeholder write callbacks from `PlaceholderElasticsearchWriteCallbacks`, and overrides `Create` and `Update` on the concrete type when the envelope cannot supply required inputs (for example `security_user`, which reads write-only password material from `Config`). Those overrides SHOULD still refresh final state from a read (or equivalent) after mutations where applicable.
