...
The workflow SHALL request permissions sufficient to read the repository, **submit pull request reviews and review comments**, and **push commits** to the pull request branch via **`push-to-pull-request-branch`**. At minimum this SHALL include **`contents: write`** and **`pull-requests: write`** unless the agentic compiler emits a narrower equivalent that still allows those operations.

#### Scenario: Push safe output is permitted

- GIVEN the agent archives the change and produces a commit on the PR branch
- WHEN `push-to-pull-request-branch` runs
- THEN the token SHALL have authority to push to the PR head branch under normal repository settings
...
4. **Any** file under `openspec/changes/<id>/` (non-archive) has a status other than **`modified`** among the set the workflow cares about (e.g. **`removed`** or **`renamed`**) if the workflow instructions adopt **modified-only** strictness as in design.
