...
**Unknown panel-type preservation**: When the read path encounters a panel whose `type` does not match any typed configuration block and is not authored through `config_json`, the resource SHALL preserve the panel's `id`, `grid`, `type`, and the panel's full raw API configuration payload in state. The preserved payload SHALL be stored privately on the panel model and SHALL NOT be exposed as a user-facing Terraform attribute. On subsequent writes that include the unchanged unknown panel, the resource SHALL re-emit the preserved payload verbatim. If a configuration declares a panel whose `type` matches no typed block and no preserved payload exists in state, the resource SHALL return the existing "unsupported panel type" error diagnostic.
