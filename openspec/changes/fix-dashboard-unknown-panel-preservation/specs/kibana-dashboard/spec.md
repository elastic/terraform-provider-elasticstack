...
**Unknown panel-type preservation**: When the read path encounters a panel whose `type` does not match any typed configuration block and is not authored through `config_json`, the resource SHALL preserve the panel's `id`, `grid`, `type`, and the panel's full raw API configuration payload in state. The preserved payload SHALL be stored in the panel's existing `config_json` attribute (which is `Optional: true, Computed: true`), reusing that attribute for storage rather than adding a new private field. This is an intentional implementation decision: it avoids introducing a new unexposed attribute and leverages the existing semantic-equality normalization path already in place for `config_json`. On subsequent writes that include the unchanged unknown panel, the resource SHALL re-emit the preserved payload verbatim through the same `config_json` codepath used for authored `config_json` panels. If a configuration declares a panel whose `type` matches no typed block and no preserved payload exists in state, the resource SHALL return the existing "unsupported panel type" error diagnostic.
