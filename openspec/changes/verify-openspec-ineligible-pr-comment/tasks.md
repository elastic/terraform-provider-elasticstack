...
- [x] 3.1 Run `gh aw compile .github/workflows/openspec-verify-label.md` and commit the updated `.github/workflows/openspec-verify-label.lock.yml`.

## 4. Update the delta spec

- [x] 4.1 Ensure `openspec/changes/verify-openspec-ineligible-pr-comment/specs/ci-aw-openspec-verification/spec.md` is aligned with the requirement added in this change (see delta spec).

## 5. Validate

- [x] 5.1 Run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate verify-openspec-ineligible-pr-comment --type change` and resolve any issues.
- [x] 5.2 Run the unit test suite for the new script: `node --test .github/scripts/workflows/openspec-verify/comment-ineligible.test.mjs`.
