...
- [ ] 2.4 Implement `read.go`: construct space-aware path via `kibanautil.BuildSpaceAwarePath`; call `GetSourceMapsWithResponse`; paginate through all pages; locate artifact by `id` from state; if not found, remove resource from state; if found, set `id` (all other attributes are write-only and are not updated on read because the API does not return original source map content).
