...
- [ ] 2.5 In `convertThreatToModel` (`models_from_api_type_utils.go`), change the nil/empty-slice branches for `technique` and `subtechnique` to return `types.ListValueMust(getThreatTechniqueElementType(), []attr.Value{})` and `types.ListValueMust(getThreatSubtechniqueElementType(), []attr.Value{})` respectively, instead of `types.ListNull(...)`.
...
- [ ] 3.5 Add a unit test for the `convertThreatToModel` change: verify that a non-empty threat with nil/empty `Technique` produces an empty-list `Technique` (not null), and a technique entry with nil/empty `Subtechnique` produces an empty-list `Subtechnique` (not null).
