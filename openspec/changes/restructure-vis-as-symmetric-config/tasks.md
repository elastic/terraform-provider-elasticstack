...
- [x] 5.3 Panel-level chart fields on `panelModel` kept as zombies tagged `tfsdk:"-"` (schema removed in 5.1; PF conversion must omit them until task 6). Rewriting each chart-block converter to take `*vizByValueModel` / `*lensDashboardAppByValueModel` and fixing `mapPanelFromAPI` to populate `VizConfig.ByValue` instead is deferred to task 6 (Option B).
