...
7. The active OpenSpec change `migrate-index-template-to-plugin-framework` is touched only if it lands second; archived changes are not touched.
...
### Non-decision: ordering vs. `migrate-index-template-to-plugin-framework`

The user explicitly opted to handle ordering. This change does not declare a hard sequencing requirement against the in-flight index-template migration; whichever lands second performs the trivial `resourcecore` → `entitycore` rename in its own diff.
...
None. Open threads from the explore session are resolved by the proposal: single change (A+B+C), `XBase` naming, APM excluded from the data-source pilot, minimal-diff pilots (`enrich_policy`, `spaces`, `enrollment_tokens`), spec capability renamed, ordering vs. the index-template change deferred to the implementer.
