...
- [ ] 1.1 In `openspec/specs/ci-build-lint-test/spec.md`'s `## Purpose` section, remove the phrase
...
- [ ] 2.4 Apply the `## MODIFIED Requirements` block for "Job permissions (REQ-028–REQ-029)": rename
...
- [ ] 2.5 Apply the `## MODIFIED Requirements` block for "Change classification gate
...
- [ ] 2.6 Apply the `## MODIFIED Requirements` block renaming "Test validation job (REQ-034–REQ-036)"
...
- [ ] 2.7 Apply the `## MODIFIED Requirements` block for "Auto-approve job (REQ-018–REQ-021)": depend
      ...
      fails" scenario.
- [ ] 2.8 Apply the `## REMOVED Requirements` entry for "Generated changelog pull requests can reach
      auto-approve without full CI": delete the requirement and its two scenarios.
- [ ] 2.9 Apply the `## REMOVED Requirements` entry for "Changelog-only bypass remains narrowly
...
- [ ] 3.2 After sync, confirm `openspec/specs/ci-build-lint-test/spec.md` no longer contains the
...
- [ ] 3.3 Confirm `openspec/specs/ci-pr-auto-approve/spec.md` is unchanged by this change — the
