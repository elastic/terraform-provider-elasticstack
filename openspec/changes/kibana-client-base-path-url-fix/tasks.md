...
- [x] 2.1 In `internal/clients/kibanautil/spaces_test.go`, extend `TestSpaceAwarePathRequestEditor` with cases covering base-path configurations:
...
- [x] 2.2 Run `go test ./internal/clients/kibanautil/...` and confirm all tests pass.

## 3. Validation

- [x] 3.1 Run `make build` to ensure the change compiles.
- [x] 3.2 Run `make check-lint` to ensure lint passes.
- [x] 3.3 Run `OPENSPEC_TELEMETRY=0 ./node_modules/.bin/openspec validate kibana-client-base-path-url-fix --type change` and confirm the change is valid.
