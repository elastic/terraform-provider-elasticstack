...
- **WHEN** `Read` has successfully deserialized config, resolved the scoped client, and resolved the read identity
- **THEN** the concrete read function SHALL be called with the scoped client, the resolved identity (`resourceID`, plus `spaceID` for Kibana), and the model `T`
- **AND** its returned `(T, bool, diag.Diagnostics)` SHALL drive state setting (when `found == true`) and centralized not-found handling (when `found == false`)
