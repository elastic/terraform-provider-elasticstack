...
- [x] 1.1 Create package `internal/elasticsearch/index/indexmappings/` with files:
...
- [x] 2.1 Define the following attributes in `getSchemaFactory`:
...
- [x] 2.2 Confirm `elasticsearch_connection` block is injected by `entitycore.ElasticsearchResource` scaffold (not manually added)
- [x] 2.3 Set the resource schema `Description` to: "Manage a user-declared subset of index mappings on an existing Elasticsearch index. Destroy is a no-op — field mappings are not removed."


## 3. Create operation (`create.go`)

- [x] 3.1 Implement `createIndexMappings(ctx, client, req)`:
...
- [x] 4.1 Implement `readIndexMappings(ctx, client, id, state)`:
...
- [x] 4.2 Ensure the read result uses `index.MappingsType{}` for semantic equality so unchanged mappings do not produce a diff

## 5. Update operation (`update.go`)

- [x] 5.1 Implement `updateIndexMappings(ctx, client, req)`:
...
- [x] 6.1 Implement `deleteIndexMappings(ctx, client, id, state)`:
  - Take no API action (Elasticsearch does not support removing field mappings without a reindex)
  - Return nil diagnostics
- [x] 6.2 Add a comment in `delete.go` explaining the no-op: field mappings in Elasticsearch are append-only; removing them requires a reindex outside the scope of this resource.

## 7. Provider registration

- [x] 7.1 Add `indexmappings.NewIndexMappingsResource()` to the provider's resource list in `provider.go`

## 8. Acceptance tests

- [x] 8.1 Create `internal/elasticsearch/index/indexmappings/acc_test.go` with:
...
- [x] 8.2 Create corresponding `testdata/TestAccResourceIndexMappings/*/` Terraform fixture files

## 9. Documentation

- [x] 9.1 Run `make generate` (or equivalent) to regenerate the provider documentation page for `elasticstack_elasticsearch_index_mappings`
- [x] 9.2 Verify the generated docs page renders the no-op destroy note from the schema description
