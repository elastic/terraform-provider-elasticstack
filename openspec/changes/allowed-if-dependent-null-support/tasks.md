...
- [ ] 1.2 Add an explicit `AllowedIfDependentPathEqualsOrNull` helper that allows the dependent path to equal the required value or be null/unset.
...
- [ ] 2.1 Update `internal/kibana/spaces/resource_schema.go` to replace unconditional `ConflictsWith` validation with the new conditional helper for `disabled_features` and `solution`.
