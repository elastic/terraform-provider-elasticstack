...
- [ ] 1.1 Create `internal/clients/elasticsearch/synonyms.go` with the following exported functions:
...
- [ ] 2.1 Create `internal/elasticsearch/synonyms/models.go` with:
...
- [ ] 3.1 Create `internal/elasticsearch/synonyms/resource.go`:
...
- [ ] 3.2 Create `internal/elasticsearch/synonyms/create.go` (upsert handler calling `PutSynonymSet` then setting ID to `<cluster_uuid>/<synonym_set_id>`)
- [ ] 3.3 Create `internal/elasticsearch/synonyms/read.go` (calling `GetSynonymSet`, removing from state when 404)
- [ ] 3.4 Create `internal/elasticsearch/synonyms/delete.go` (calling `DeleteSynonymSet` with clear HTTP 400 diagnostic)
