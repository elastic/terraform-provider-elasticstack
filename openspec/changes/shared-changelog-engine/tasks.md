...
- [x] 1.1 Identify the current changelog-generation logic that must move behind a shared repository-authored script interface, including release-context resolution, merged-PR discovery, changelog parsing/validation, rendering, and `CHANGELOG.md` rewriting
- [x] 1.2 Implement the shared engine so it accepts explicit mode inputs (`unreleased` or `release`, with explicit target version for release mode), resolves merged PRs through the GitHub API using the workflow token, and emits structured outputs needed by workflows
