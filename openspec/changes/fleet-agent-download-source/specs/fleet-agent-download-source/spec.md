...
The resource SHALL support `terraform import` by accepting a composite ID in the form `<space_id>/<source_id>`. The Fleet download source ID SHALL populate both `id` and `source_id`. The Space ID SHALL populate `space_ids` as a single-entry collection.
