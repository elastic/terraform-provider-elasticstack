...
- In `writeIntegration` (`create.go`), resolve `scope` once from `planModel.SpaceID`, pass a derived poll scope `spaceScope{id: scope.id, aware: false}` to the first post-install wait (scoped get-package API path with global install detection only), use `scope.id` for the subsequent `GetPackage`/`fleetPackageInstalled` checks, and pass `scope.id` into `installInSpace` (whose later wait uses strict `aware: true` target-space detection).
