...
- [x] 1.2 Add pure `resolveSpaceID(spaceID types.String) string` normalization and `resolveSpaceScope(ctx, client clients.MinVersionEnforceable, spaceID types.String, diags *diag.Diagnostics) spaceScope`, with only the latter applying the existing `supportsSpaceAwareIntegration` version-gate logic.
...
- [x] 1.5 Add table-driven unit tests for `resolveSpaceID` normalization and `fleetPackageInstalled` spaceScope semantics (global vs strict target-space detection).
...
- [x] 2.2 In `writeIntegration`, resolve the ID-only `scope := spaceScope{id: resolveSpaceID(planModel.SpaceID)}` before the install call, without an eager server capability/version check.
- [x] 2.3 Replace the buggy call `waitForFleetIntegrationInstalled(ctx, fleetClient, name, version, "", false)` with `waitForFleetIntegrationInstalled(ctx, fleetClient, name, version, scope)`. Scope the get-package API path to `scope.id` (fix for issue #4282) while keeping global install detection for this first wait.
...
- [x] 2.5 Replace `installedInTargetSpace := fleetPackageInstalled(pkg, spaceID, true)` with `fleetPackageInstalled(pkg, spaceScope{id: scope.id, aware: true})`, preserving today's deliberately strict target-space metadata check; `installInSpace` retains responsibility for capability gating and the unsupported-version warning.
...
- [x] 3.2 `internal/fleet/integration/delete.go`: same substitution in `deleteIntegration`; use `scope.id` consistently for Fleet API calls and pass it to `isInstalledInMultipleSpaces` and `deleteKibanaAssetsWithFallback`, which continue to take a plain ID string because they don't need `aware`.
...
- [ ] 4.5 Confirm the existing role privilege set (`fleet`/`fleetv2` all, scoped to the test space) works against a running 9.1+ stack — adjust only if the restricted user 403s on the legitimately-scoped path too. *(Pending revalidation on the review-fix head; the local Elastic Stack is unavailable.)*
...
- [ ] 5.4 Bring up a 9.1+ Elastic Stack and run `make testacc TESTARGS='-run TestAccReproduceIssue4282'` (or the repo's docker-testacc equivalent); confirm it fails pre-fix (403 during post-install wait) and passes post-fix. *(Pending revalidation on the review-fix head; the local Elastic Stack is unavailable.)*
- [ ] 5.5 Re-run `TestAccResourceIntegration_MultiSpaceInstall`, `TestAccResourceIntegration_MultiSpaceDelete`, and `TestAccResourceIntegration_SpaceAwareDrift` to confirm no regression from the `spaceScope` refactor. *(Pending revalidation on the review-fix head; the local Elastic Stack is unavailable.)*
