...
- `actions` (Block List) Array of automated actions taken when alerts are generated by the rule. (see [below for nested schema](#nestedblock--actions))
...
- `alerts_filter` (Block, Optional) Conditions that affect whether the action runs. If you specify multiple conditions, all conditions must be met for the action to run.

The `query` block supports a KQL string and a JSON array of Kibana filter objects. Use `filters_json = jsonencode([])` when no filters are required. Example: `alerts_filter { query { kql = "event.action : \"test\"" filters_json = jsonencode([]) } }`. (see [below for nested schema](#nestedblock--actions--alerts_filter))
- `frequency` (Block, Optional) The action frequency defines when the action runs. (see [below for nested schema](#nestedblock--actions--frequency))
...
- `query` (Block, Optional) KQL query and Kibana filter DSL conditions that determine whether the action runs. (see [below for nested schema](#nestedblock--actions--alerts_filter--query))
...
<a id="nestedblock--actions--alerts_filter--query"></a>
