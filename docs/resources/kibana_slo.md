...
- `time_window` (Block List) Currently supports `calendarAligned` and `rolling` time windows.
...
Rolling time window SLOs only consider SLI data from the last duration period as a moving window. Calendar aligned time windows align to calendar boundaries. (see [below for nested schema](#nestedblock--time_window))
