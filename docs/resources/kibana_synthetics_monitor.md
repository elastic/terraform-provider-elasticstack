...
- `kibana_spaces` (List of String) Kibana spaces in which the monitor is visible. Requires Elastic Stack 9.6.0 or later when configured. Use `["*"]` to share the monitor with all spaces. Removing a configured value clears additional visibility. The provider includes the owning `space_id` in the API request automatically.
