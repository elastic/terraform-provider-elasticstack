...
func (s *KBAPITestSuite) TestKibanaSyntheticsParameterAPI() {
	ctx := context.Background()

	pAPI := s.API.KibanaSynthetics.Parameter
	trueBool := false

	s.Run("TestKibanaSyntheticsParameterAPI", func() {
		cfg := ParameterConfig{
			Key:               "test-key",
			Value:             "test-value",
			Description:       "Test description",
			Tags:              []string{"a", "b"},
			ShareAcrossSpaces: &trueBool,
		}

		created, err := pAPI.Add(ctx, cfg)
		assert.NoError(s.T(), err)
		assert.Equal(s.T(), created.Key, cfg.Key)
		assert.Equal(s.T(), created.Value, cfg.Value)
		assert.Equal(s.T(), created.Description, cfg.Description)
		assert.Equal(s.T(), created.Tags, cfg.Tags)
		assert.Equal(s.T(), created.Namespaces, []string{"*"})
		...
		cfg.Key = "test-key-2"
		cfg.Value = "test-value-2"
		cfg.Description = "Test description 2"
		cfg.Tags = []string{"c", "d", "e"}

		updated, err := pAPI.Update(ctx, created.Id, cfg)
		assert.NoError(s.T(), err)
		assert.Equal(s.T(), updated.Id, created.Id)
		assert.Equal(s.T(), updated.Key, cfg.Key)
		assert.Equal(s.T(), updated.Value, cfg.Value)
		assert.Equal(s.T(), updated.Description, cfg.Description)
		assert.Equal(s.T(), updated.Tags, cfg.Tags)

		deleted, err := pAPI.Delete(ctx, created.Id)
		assert.NoError(s.T(), err)
		assert.Equal(s.T(), deleted, []ParameterDeleteStatus{
			{
				Id:      created.Id,
				Deleted: true,
			},
